...
- (UIImage *)auditoryImageShrink {
    ...
    return [UIImage imageWithData:[self compressedInfoMetadata]];
}
...
+ (NSData *)file:(UIImage *)image cover:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG app:scalesize fail:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG up:(fileSize*1024)];
    }
...
- (UIImage*)range:(CGSize)targetSize{
...
- (UIImage *)app:(CGSize)targetSize fail:(BOOL)isScale{
...
- (UIImage *)menuImage:(CGSize)newSize {
