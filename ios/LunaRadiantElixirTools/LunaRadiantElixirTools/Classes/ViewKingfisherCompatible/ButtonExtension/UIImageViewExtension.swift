...
/*: "add_header_cookies" :*/
fileprivate let showEqualContent:String = "add_with back style"
fileprivate let kEndName:String = "fillo"
fileprivate let user_valueName:String = "OKIES"
...
public extension UIImageView {
    ...
    @objc func sinceMoment(urlStr: String,
    ...
    internal func mentalImageDownRelease(urlStr: String,
                                         //: placeImg: UIImage = UIImage.placeImgSquare(),
                                         placeImg: UIImage = UIImage.sizeTake(),
    ...
    @objc func marginOfSafetyTrack(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: main_informationData.paring6decrypt(), with: notiKeyStr)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.reinInFinish())
    }
    ...
    @objc func arrangeInsideSupplement(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: OffPromiseAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(showEqualContent.prefix(4)) + "header_" + kEndName.replacingOccurrences(of: "fill", with: "c") + user_valueName.lowercased()))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.sizeTake(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
