...
/*: "recharge" :*/
fileprivate let dataImageName:[Character] = ["r","e","c","h","a","r","g","e"]

/*: "recharge_ :*/
fileprivate let data_rangeIndexKey:String = "RECH"

/*: ".png" :*/
fileprivate let show_withPath:[Character] = [".","p","n","g"]

/*: "_ :*/
fileprivate let noti_userUrl:String = "succeed"

/*: .png" :*/
fileprivate let kPicFormat:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let main_modelText:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let kCloudId:[Character] = [".","j","p","g"]
...
enum CreateImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class TalkingItemButton: UIButton {
    ...
    var imageAlignment: CreateImageAlignment = .left
...
extension UIButton {
    ...
    func clickTitle(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        equalAdd(interval: interval, topOffset: 0)
    }
    ...
    func equalAdd(interval: CGFloat, topOffset: CGFloat) {
    ...
    func unsloped(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func mastheadState(color: UIColor, forState: UIControl.State) {
    ...
    func visualCommunicationUser(urlStr: String,
                                 //: placeImg: UIImage? = UIImage.placeImgSquare(),
                                 placeImg: UIImage? = UIImage.sizeTake(),
                                 ...
                                 loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: main_informationData.paring6decrypt(), with: notiKeyStr)
    ...
    func cramFor(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((String(dataImageName))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (String(dataImageName)), with: (data_rangeIndexKey.lowercased() + "arge_") + "\(showNoName)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.visualCommunicationUser(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(show_withPath))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(show_withPath)), with: "_\(showNoName)" + (String(kPicFormat)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(main_modelText))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(main_modelText)), with: "_\(showNoName)" + (String(kCloudId)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.visualCommunicationUser(urlStr: newUrl, placeImg: nil)
    }
