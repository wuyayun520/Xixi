...
/*: "http://stat. :*/
fileprivate let constBeautyFormat:String = "http:/show main"

/*: .com" :*/
fileprivate let k_fromPath:String = "self temp add info.com"

/*: "http://" :*/
fileprivate let userMethodPath:String = "http://all view succeed"

/*: "https://" :*/
fileprivate let dataViewName:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let constLockUrl:String = "http://video to"
fileprivate let const_thumbMsg:String = "title type count to letm."

/*: "http:// :*/
fileprivate let show_whoTitle:String = "http:/string size type video class"
fileprivate let notiReceiveTitle:[Character] = ["/"]

/*: "-1" :*/
fileprivate let appVideoMsg:String = "-finish"

/*: "3,90" :*/
fileprivate let kDirectionId:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let user_toContent:String = "1table0"

/*: "MM-dd-yyyy" :*/
fileprivate let appFromLoopFormat:[Character] = ["M","M","-","d","d","-"]
fileprivate let notiErrorMsg:String = "mapmapmapmap"

/*: "id" :*/
fileprivate let noti_blockFrameName:String = "ID"
...
class ContentBlockReactiveCompatible: NSObject {
    ...
    var requestType: EditRateRequestType?
    ...
    override init() {
        ...
        self.requestServer = OffPromiseAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func arrayInput() -> Bool {
...
struct FirstHandyJSON: HandyJSON {
    //: var errno: Int!
...
struct DrawMeasurable: HandyJSON {
    ...
    var CSConfig = ConfigModel() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(constBeautyFormat.prefix(6)) + "/stat.") + "\(notiKeyStr)" + (String(k_fromPath.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(userMethodPath.prefix(7)))) && !statDomain.hasPrefix((String(dataViewName))) {
                //: return "http://".appending(statDomain)
                return (String(userMethodPath.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(constLockUrl.prefix(7)) + String(const_thumbMsg.suffix(2))) + "\(notiKeyStr)" + (String(k_fromPath.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(show_whoTitle.prefix(6)) + String(notiReceiveTitle)) + "\(urlDomain)"
        }
    }
...
public class RequestModelType: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [PartTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [PartTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [PartTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [BlockTransformable]() // vip皮肤
    ...
    var tabPageBanner = [TopHandyJSON]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = WithLimitModel() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class DeleteUserModel: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [ControlMeasurable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [ControlMeasurable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (kDirectionId.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.recordEvent(self.birthday ?? (user_toContent.replacingOccurrences(of: "table", with: "99") + "-01-01"), dateFormat: (String(appFromLoopFormat) + notiErrorMsg.replacingOccurrences(of: "map", with: "y")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.components().year else { return "0" }
...
struct DrawHandyJSON: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct ControlMeasurable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func finishLiveCurrent(compareTo: ControlMeasurable) -> Bool {
        //: return
        return
...
struct PathHandyJSON: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [ReportMeasurable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [ReportMeasurable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct ReportMeasurable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
...
struct BlockTransformable: HandyJSON {
...
struct WithLimitModel: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
