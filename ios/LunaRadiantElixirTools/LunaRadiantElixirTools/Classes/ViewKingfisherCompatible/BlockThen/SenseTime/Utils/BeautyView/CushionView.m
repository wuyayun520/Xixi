...
Byte user_pictureText[] = {32, 45, 28, 14, 50, 149, 250, 87, 192, 100, 213, 131, 91, 143, 79, 73, 93, 18, 50, 83, 88, 77, 74, 77, 71, 69, 88, 77, 83, 82, 18, 56, 69, 84, 18, 56, 83, 18, 49, 83, 72, 77, 74, 93, 18, 41, 92, 84, 83, 87, 89, 86, 73, 18, 52, 83, 77, 82, 88, 138};

//: camera_focus_red
Byte main_femaleData[] = {93, 16, 22, 11, 9, 49, 54, 33, 210, 71, 252, 77, 75, 87, 79, 92, 75, 73, 80, 89, 77, 95, 93, 73, 92, 79, 78, 219};

//: FaceActionEvent_Interval_1_second
Byte notiEventOfUniqueTitle[] = {60, 33, 6, 13, 254, 239, 214, 79, 145, 192, 179, 52, 209, 64, 91, 93, 95, 59, 93, 110, 99, 105, 104, 63, 112, 95, 104, 110, 89, 67, 104, 110, 95, 108, 112, 91, 102, 89, 43, 89, 109, 95, 93, 105, 104, 94, 50};

//: hasFace
Byte mainMediaId[] = {52, 7, 92, 5, 155, 12, 5, 23, 234, 5, 7, 9, 209};

//: model
Byte constCustomTitle[] = {85, 5, 45, 9, 45, 243, 43, 166, 2, 64, 66, 55, 56, 63, 227};

//: M_SenseME_Face_Video_5.3.4
Byte mainDataTableId[] = {60, 26, 40, 4, 37, 55, 43, 61, 70, 75, 61, 37, 29, 55, 30, 57, 59, 61, 55, 46, 65, 60, 61, 71, 55, 13, 6, 11, 6, 12, 150};

//: FaceActionEvent_Interval_12_frame
Byte k_sessionStr[] = {61, 33, 24, 13, 2, 241, 55, 54, 180, 223, 114, 199, 7, 46, 73, 75, 77, 41, 75, 92, 81, 87, 86, 45, 94, 77, 86, 92, 71, 49, 86, 92, 77, 90, 94, 73, 84, 71, 25, 26, 71, 78, 90, 73, 85, 77, 108};
...
//
//#import "ViewFatalCacheCost.h"
//#import "FilterNameSimulationThen.h"
//ST_MOBILE
...
@interface CushionView ()<WhiteOver,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) VanguardLicHelper * lichelper;
@property (nonatomic, strong) VanguardLicHelper * lichelper;
//: @property (nonatomic, strong) ParamDrawReactiveCompatible * ParamUtil;
@property (nonatomic, strong) ParamDrawReactiveCompatible * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [VanguardLicHelper share];
        self.lichelper = [VanguardLicHelper share];
        //: self.ParamUtil = [[ParamDrawReactiveCompatible alloc]init];
        self.ParamUtil = [[ParamDrawReactiveCompatible alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper imageOf]) {
            ...
            //: [[ViewFatalCacheCost share] getLocalModel];
            [[ViewFatalCacheCost share] addIn];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(userred:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:StringFromPathData(user_pictureText)
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notices:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ageChanged:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initOralCommunication:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper imageOf]) {
            ...
            [self initSeparate:sessionPreset];

        //: } else {
        } else {
            //: [VanguardLicHelper.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [VanguardLicHelper.share upwardsViewWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initSeparate:sessionPreset];
...
- (void)initSeparate:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notices:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ageChanged:)
...
- (void)strainSumimate {
    ...
    //: if ([ViewFatalCacheCost share].filterSuite.filterModelPath.length > 0) {
    if ([ViewFatalCacheCost share].filterSuite.filterModelPath.length > 0) {
        //: FilterNameSimulationThen* model = [[ViewFatalCacheCost share] getFilterDataModel];
        FilterNameSimulationThen* model = [[ViewFatalCacheCost share] humiliation];
        //: if (model) {
        if (model) {
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera pleasingnessFrame:self.previewCenter throwOverboard:self.previewFrame];
}
...
- (void)filter:(BOOL)isFullScreen {
    ...
    [self bastardy:width size:height];
}
/// 设置视频通话预览的采集大小
...
- (void)photo {
    ...
    _stCamera = [[CameraNetTo alloc] initWithManager:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          message:self.currentSessionPreset
                                                     //: fps:fps
                                                     pauseOf:fps
                                           //: needYuvOutput:NO];
                                           gift:NO];
...
- (void)bastardy:(CGFloat)width size:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera overcharge:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   countRelation:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[FlushStraddleView alloc] initWithDataFull:previewRect plus:previewContext];
        }
...
- (void)addMenu {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(activityFrame) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(everyGiftCount) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pauseIndex) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sampleWant) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)exaggerate:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)textFrame {
    ...
    [self exaggerate:self.motionManager.accelerometerData];
...
- (void)filter {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [MixInReactiveCompatible.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [MixInReactiveCompatible.endName stringByAppendingPathComponent:StringFromPathData(mainDataTableId)];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:StringFromPathData(constCustomTitle)];
...
- (BOOL)with:(CVPixelBufferRef)pixelBuffer {
...
- (void)userred:(NSNotification *)sender {
    ...
    if ([tapGesture isKindOfClass:UITapGestureRecognizer.class]) {
        ...
        [self device:tapGesture];
    }

}

//: - (void)tapScreen:(UITapGestureRecognizer *)tapGesture {
- (void)device:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera pleasingnessFrame:point throwOverboard:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:StringFromPathData(main_femaleData)];
...
- (void)notices:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[FilterNameSimulationThen class]]) {
    if ([notice.object isKindOfClass:[FilterNameSimulationThen class]]) {
        //: FilterNameSimulationThen* model = (FilterNameSimulationThen*)notice.object;
        FilterNameSimulationThen* model = (FilterNameSimulationThen*)notice.object;
        //: [self handleFilterChanged:model];
        [self custom:model];
    }
...
//: - (void)handleFilterChanged:(FilterNameSimulationThen *)model {
- (void)custom:(FilterNameSimulationThen *)model {
...
- (void)ageChanged:(NSNotification*) notice {
    //: FilterNameSimulationThen* model = (FilterNameSimulationThen*)notice.object;
    FilterNameSimulationThen* model = (FilterNameSimulationThen*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)key {
    ...
    //: FilterEntiretycostModel *beautyValueCache = ViewFatalCacheCost.share.currentBeautySuite;
    FilterEntiretycostModel *beautyValueCache = ViewFatalCacheCost.share.fileHandle;

    // 设置美白参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self message:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self message:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self message:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self message:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self message:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self message:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil beautifyWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];

}

//: - (float)rangeMappedValueFrom:(float) rawValue {
- (float)message:(float) rawValue {
...
void mobileTargetFace(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void mouthFace(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)quantityimateTo:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       data:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(head:noneEffectStart:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate head:self noneEffectStart:sampleBuffer];
            }
...
- (CVPixelBufferRef)icon:(CVPixelBufferRef)pixelBuffer {
...
- (void)pause:(AVCaptureOutput *)captureOutput capture:(CMSampleBufferRef)sampleBuffer local:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self textFrame];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromPathData(k_sessionStr) object:nil userInfo: @{StringFromPathData(mainMediaId): @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromPathData(notiEventOfUniqueTitle) object:nil userInfo: @{StringFromPathData(mainMediaId): @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        mobileTargetFace(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self with:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self quantityimateTo:sampleBuffer data:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            mouthFace(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview user:textureResult];
            }
        }
...
Byte * PathDataToCache(Byte *data) {
    ...
    Byte dump = data[2];
    int size = data[3];
    if (!each) return data + size;
    for (int i = size; i < size + live; i++) {
        int value = data[i] + dump;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
