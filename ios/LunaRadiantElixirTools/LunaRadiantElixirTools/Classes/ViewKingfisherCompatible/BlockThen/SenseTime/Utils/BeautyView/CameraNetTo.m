...
@interface AllFilterData : NSObject

+ (instancetype)sharedInstance;

//: 请打开相机权限
@property (nonatomic, copy) NSString *kDumpValue;

//: Height
@property (nonatomic, copy) NSString *data_logPath;

//: Width
@property (nonatomic, copy) NSString *kProcessStr;

@end

@implementation AllFilterData

+ (instancetype)sharedInstance {
    static AllFilterData *instance = nil;
...
- (Byte *)AllFilterDataToByte:(StructAllFilterData *)data {
    for (int i = 0; i < data->render; i++) {
        data->sumServerRef[i] ^= data->circulate;
    }
    data->sumServerRef[data->render] = 0;
	if (data->render >= 3) {
		data->add = data->sumServerRef[0];
		data->renderTable = data->sumServerRef[1];
		data->status = data->sumServerRef[2];
	}
    return data->sumServerRef;
}

- (NSString *)StringFromAllFilterData:(StructAllFilterData *)data {
    return [NSString stringWithUTF8String:(char *)[self AllFilterDataToByte:data]];
}

//: Height
- (NSString *)data_logPath {
    if (!_data_logPath) {
        StructAllFilterData value = (StructAllFilterData){59, (Byte []){115, 94, 82, 92, 83, 79, 86}, 6, 220, 193, 226};
        _data_logPath = [self StringFromAllFilterData:&value];
    }
    return _data_logPath;
}

//: 请打开相机权限
- (NSString *)kDumpValue {
    if (!_kDumpValue) {
        StructAllFilterData value = (StructAllFilterData){241, (Byte []){25, 94, 70, 23, 120, 98, 20, 77, 113, 22, 106, 73, 23, 109, 75, 23, 108, 114, 24, 104, 97, 82}, 21, 73, 49, 67};
        _kDumpValue = [self StringFromAllFilterData:&value];
    }
    return _kDumpValue;
}

//: Width
- (NSString *)kProcessStr {
    if (!_kProcessStr) {
        StructAllFilterData value = (StructAllFilterData){204, (Byte []){155, 165, 168, 184, 164, 176}, 5, 121, 144, 127};
        _kProcessStr = [self StringFromAllFilterData:&value];
    }
    return _kProcessStr;
}
...
@interface CameraNetTo () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation CameraNetTo

//: - (instancetype)initWithDevicePosition:(AVCaptureDevicePosition)iDevicePosition
- (instancetype)initWithManager:(AVCaptureDevicePosition)iDevicePosition
                        //: sessionPresset:(AVCaptureSessionPreset)sessionPreset
                        message:(AVCaptureSessionPreset)sessionPreset
                                   //: fps:(int)iFPS
                                   pauseOf:(int)iFPS
                         //: needYuvOutput:(BOOL)needYuvOutput
                         gift:(BOOL)needYuvOutput
{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
        ...
        self.videoDevice = [self begin:iDevicePosition];
...
- (void)pleasingnessFrame:(CGPoint)point throwOverboard:(CGRect)frame
{
    ...
    [self telegram:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           hide:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                setupAt:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)telegram:(AVCaptureFocusMode)focusMode hide:(AVCaptureExposureMode)exposureMode setupAt:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self changeBean:^(AVCaptureDevice *captureDevice) {
...
- (void)changeBean:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setTitleureBean:(float)value
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self begin:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self image]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[AllFilterData sharedInstance].kProcessStr] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[AllFilterData sharedInstance].data_logPath] floatValue];
...
- (CGRect)overcharge:(CGRect)rect countRelation:(BOOL)bScaleToFit
{
    //: CGRect rectRet = rect;
    CGRect rectRet = rect;
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[AllFilterData sharedInstance].kProcessStr] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[AllFilterData sharedInstance].data_logPath] floatValue];
...
- (BOOL)image
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[AllFilterData sharedInstance].kDumpValue
...
- (AVCaptureDevice *)begin:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(pause:capture:local:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate pause:captureOutput capture:sampleBuffer local:connection];
        }
    }
