...
//: STDisplayShader
@property (nonatomic, copy) NSString *main_startMessage;
...
+ (instancetype)sharedInstance {
    static RebuildData *instance = nil;
...
- (Byte *)RebuildDataToCache:(Byte *)data {
    int tableFemale = data[0];
    Byte cancelResume = data[1];
    int title = data[2];
    for (int i = title; i < title + tableFemale; i++) {
        int value = data[i] - cancelResume;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[title + tableFemale] = 0;
    return data + title;
}
...
//: STDisplayShader
- (NSString *)main_startMessage {
    if (!_main_startMessage) {
        Byte value[] = {15, 10, 13, 10, 212, 46, 100, 156, 132, 44, 238, 47, 55, 93, 94, 78, 115, 125, 122, 118, 107, 131, 93, 114, 107, 110, 111, 124, 236};
        _main_startMessage = [self StringFromRebuildData:value];
    }
    return _main_startMessage;
}
...
@implementation FlushStraddleView
...
- (instancetype)initWithDataFull:(CGRect)frame plus:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self detoxify:[RebuildData sharedInstance].main_startMessage
                //: fragmentShader:@"STDisplayShader"
                currentFill:[RebuildData sharedInstance].main_startMessage
                    //: forProgram:&stDisplayProgram];
                    upToValue:&stDisplayProgram];
...
- (BOOL)detoxify:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          currentFill:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              upToValue:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self equalAvailable:&vertexShader translate:0x8B31 object:mainContextValue]) {
    ...
    if (![self equalAvailable:&fragShader translate:0x8B30 object:app_frameFrontName]) {
...
- (BOOL)equalAvailable:(GLuint *)shader translate:(GLenum)type object:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
