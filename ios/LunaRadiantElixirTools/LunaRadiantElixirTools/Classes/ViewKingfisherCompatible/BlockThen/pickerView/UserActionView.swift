...
/*: "Select a date" :*/
fileprivate let noti_timeStr:String = "Selectreturn model resistance"
fileprivate let main_nameUrl:String = "all"

/*: M" :*/
fileprivate let userResultModelValue:[Character] = ["M"]

/*: D" :*/
fileprivate let show_textGroupMsg:[Character] = ["D"]

/*: Y" :*/
fileprivate let const_labelFormat:[Character] = ["Y"]
...
protocol BeanViewDelegate: NSObjectProtocol {
...
class UserActionView: BlockHandPickThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: BeanViewDelegate?
    ...
    override func glow() {
        ...
        titleString = (String(noti_timeStr.prefix(6)) + " a dat" + main_nameUrl.replacingOccurrences(of: "all", with: "e")).localized
        ...
        someFile(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func someFile(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func valueMove(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.shared(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.shared(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return butylate(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func butylate(year: Int, month: Int) -> Int {
