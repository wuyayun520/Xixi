...
fileprivate let app_modelStr:[UInt8] = [0xab,0xac,0xab,0xb6,0xea,0xa1,0xad,0xa6,0xa7,0xb0,0xf8,0xeb,0xe2,0xaa,0xa3,0xb1,0xe2,0xac,0xad,0xb6,0xe2,0xa0,0xa7,0xa7,0xac,0xe2,0xab,0xaf,0xb2,0xae,0xa7,0xaf,0xa7,0xac,0xb6,0xa7,0xa6]

private func emptySelf(content num: UInt8) -> UInt8 {
    return num ^ 194
}

/*: "FFD855" :*/
fileprivate let show_myValueName:[Character] = ["F"]
fileprivate let user_labVideoValue:String = "FD8main"
...
typealias CropSlidingHandler = (_ direction: ModifySlideDirection) -> Void
//: typealias CropSlidingEndHandler = () -> Void
typealias CropSlidingEndHandler = () -> Void

//: enum TalkingSlideDirection {
enum ModifySlideDirection {
    //: case left
    case left
    //: case right
    case right
...
class VisualCommunicationThen: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - appKeyPath
                ...
                self.gross(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_modelStr.map{emptySelf(content: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(show_myValueName) + user_labVideoValue.replacingOccurrences(of: "main", with: "55")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: const_lineStr, y: 0, width: appKeyPath, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - appKeyPath, y: 0, width: appKeyPath, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.buttonThemeTip(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: const_lineStr, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.buttonThemeTip(0, 0, 0, 0.6)
...
extension VisualCommunicationThen {
    ...
    func afterwardState() {
        ...
        self.leftSilder.frame = CGRect(x: const_lineStr, y: 0, width: appKeyPath, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - appKeyPath, y: 0, width: appKeyPath, height: self.frame.size.height)
        //: resetIndicator()
        yearSearch()
    }

    //: func needShowIndicator(isShow: Bool) {
    func messageAll(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func gross(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - appKeyPath,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - appKeyPath)
    }
}
...
extension VisualCommunicationThen {
    ...
    @objc func outContent(gesture: UIPanGestureRecognizer) {
    ...
    @objc func windowDown(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension VisualCommunicationThen {
    ...
    private func observerSubviews() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(outContent(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(windowDown(gesture:)))
