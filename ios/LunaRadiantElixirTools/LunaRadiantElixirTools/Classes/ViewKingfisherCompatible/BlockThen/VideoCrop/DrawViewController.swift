...
fileprivate let constViewTitle:[UInt8] = [0x4e,0x49,0x4e,0x53,0xf,0x44,0x48,0x43,0x42,0x55,0x1d,0xe,0x7,0x4f,0x46,0x54,0x7,0x49,0x48,0x53,0x7,0x45,0x42,0x42,0x49,0x7,0x4e,0x4a,0x57,0x4b,0x42,0x4a,0x42,0x49,0x53,0x42,0x43]

private func priceShared(to num: UInt8) -> UInt8 {
    return num ^ 39
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let app_sendValue:String = "frame send content databtnJ"
fileprivate let showModelSourceUrl:String = "area viewB_etihw_k"
fileprivate let mainContentUrl:[Character] = ["c","a","b"]
fileprivate let dataBackUrl:String = "false add_shadow"

/*: "Edit" :*/
fileprivate let app_useText:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let mainMiniData:String = "video any show succeedFinish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let constNoText:String = "string app return on dismissbtnY"
fileprivate let k_momentClickTitle:String = "div_in add type type"

/*: "Cancel" :*/
fileprivate let dataTempRangeMessage:String = "view height filter heightCancel"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let app_orientationViewValue:String = "equal heightbgQk"
fileprivate let noti_cloudMatchMessage:[Character] = ["i","v","_","t","o","a","s","t"]

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let constLevelName:[UInt8] = [0xd9,0xe5,0xe8,0xad,0xfb,0xe4,0xe9,0xe8,0xe2,0xad,0xe1,0xe8,0xe3,0xea,0xf9,0xe5,0xad,0xfe,0xe5,0xe2,0xf8,0xe1,0xe9,0xad,0xef,0xe8,0xad,0xa8,0xcd,0xa0,0xa8,0xcd,0xad,0xfe,0xe8,0xee,0xe2,0xe3,0xe9,0xfe,0xa3,0xad,0xdd,0xe1,0xe8,0xec,0xfe,0xe8,0xad,0xe4,0xe3,0xf9,0xe8,0xff,0xee,0xe8,0xfd,0xf9,0xad,0xe4,0xf9]
...
typealias VideoEditBlock = (_ videoInfo: TextHandyJSON?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let notiMarginImageValue: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class DrawViewController: DropBaseViewController {
    ...
    private var slider: UnhingeViewDelegate?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = TextHandyJSON() // 编辑结果模型
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if MakeSocketDelegate.shared.isInfo == true { // 正在音视频通话，静音播放
    ...
    init(style: ScreenEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (appUseMessage - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constViewTitle.map{priceShared(to: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.barName(name: (String(app_sendValue.suffix(4)) + "QbEA" + String(showModelSourceUrl.suffix(9)) + String(mainContentUrl) + String(dataBackUrl.suffix(7)))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = CapacityLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(dorsalVertebra), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(app_useText)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.readModeAchromaticColour(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.statusDataMove(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = CapacityLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(mainMiniData.suffix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.finishFont(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.readModeAchromaticColour()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(undersideWithoutAccomplishment), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.barName(name: (String(constNoText.suffix(4)) + "Do2AS_oe" + String(k_momentClickTitle.prefix(4)) + "play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(modelElement(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(dataTempRangeMessage.suffix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(noUp), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.barName(name: (String(app_orientationViewValue.suffix(4)) + "RTbF_oed" + String(noti_cloudMatchMessage))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: constLevelName.map{$0^141}, encoding: .utf8)!.arguments(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: StatusDateInfoView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = StatusDateInfoView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension DrawViewController {
    ...
    private func behindTap(animated _: Bool = true) {
    ...
    @objc func sendView() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: appUseMessage - 2 * 26, height: kStatusName - 46 - 127 - constBarStr)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = appUseMessage / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kStatusName, width: appUseMessage, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = UnhingeViewDelegate(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: notiMarginImageValue)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - constBarStr)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: ModifySlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func undersideWithoutAccomplishment() {
        //: guard self.videoURL != nil else { return }
        guard self.videoURL != nil else { return }
        ...
        if self.videoEditHandler != nil {
            ...
            AttributeSignatureProvider.shared.endMan(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.behindTap()
    ...
    @objc func noUp() {
        ...
        if self.currStyle == .intimateVideo {
            ...
            self.undersideWithoutAccomplishment()
        }
    }
    ...
    @objc func modelElement(sender: UIButton) {
...
extension DrawViewController {
    ...
    @objc private func basicCognitiveProcessMain() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension DrawViewController {
    ...
    private func somebody() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(playback), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
    ...
    private func transactionUp() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + constBarStr)
        }
    ...
    private func notMethodNotification() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(tent),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(basicCognitiveProcessMain),
