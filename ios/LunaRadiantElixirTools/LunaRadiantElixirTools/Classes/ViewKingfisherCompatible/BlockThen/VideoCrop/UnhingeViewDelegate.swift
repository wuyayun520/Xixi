...
fileprivate let constGroupSectionFieldName:[UInt8] = [0xa0,0xa7,0xa0,0xbd,0xe1,0xaa,0xa6,0xad,0xac,0xbb,0xf3,0xe0,0xe9,0xa1,0xa8,0xba,0xe9,0xa7,0xa6,0xbd,0xe9,0xab,0xac,0xac,0xa7,0xe9,0xa0,0xa4,0xb9,0xa5,0xac,0xa4,0xac,0xa7,0xbd,0xac,0xad]

private func targetAdd(view num: UInt8) -> UInt8 {
    return num ^ 201
}
...
typealias SlidingHandler = (_ direction: ModifySlideDirection) -> Void
...
class UnhingeViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (const_lineStr - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constGroupSectionFieldName.map{targetAdd(view: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.toCell(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: VisualCommunicationThen = {
        ...
        let overlay = VisualCommunicationThen(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension UnhingeViewDelegate {
    ...
    private func toCell(point: CGFloat) -> UIImage {
...
extension UnhingeViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = const_lineStr - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.messageAll(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.messageAll(isShow: true)
    }
...
extension UnhingeViewDelegate {
    ...
    private func inputSubviews() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(const_lineStr)
