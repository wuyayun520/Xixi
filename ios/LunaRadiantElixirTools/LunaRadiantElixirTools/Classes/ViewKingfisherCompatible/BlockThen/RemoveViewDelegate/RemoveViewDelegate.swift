...
protocol ItemAtNavigationDelegate: AnyObject {
...
class RemoveViewDelegate: UIViewController {
    ...
    weak var delegate: ItemAtNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = MixInReactiveCompatible.getScreenHeight() - (topDistance + kTitle_H)
        var height = MixInReactiveCompatible.pullBlock() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= appDeviceErrorNewFormat
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: MixInReactiveCompatible.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: MixInReactiveCompatible.statusByPlay(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(MixInReactiveCompatible.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(MixInReactiveCompatible.statusByPlay()), height: 0)
        ...
        scrollView.semanticContentAttribute = CapacityLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: MixInReactiveCompatible.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: MixInReactiveCompatible.statusByPlay()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: MixInReactiveCompatible.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: MixInReactiveCompatible.statusByPlay()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = CapacityLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(titleShow), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = MixInReactiveCompatible.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = MixInReactiveCompatible.statusByPlay() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(MixInReactiveCompatible.getScreenWidth()-originX-titleWidth)
            let x = CapacityLanguageManager.shared.direction == .leftToRight ? originX:(MixInReactiveCompatible.statusByPlay()-originX-titleWidth)
...
extension RemoveViewDelegate {
    //: @objc private func titleButtonClick(_ sender: UIButton) {
    @objc private func titleShow(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        componentButton(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * MixInReactiveCompatible.getScreenWidth()
        let offset_x = CGFloat(index) * MixInReactiveCompatible.statusByPlay()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        randomSampleRelease(withIndex: index)
        //: seletedViewController(withIndex: index)
        changeBoot(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func randomSampleRelease(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * MixInReactiveCompatible.getScreenWidth()
        let origin_x = CGFloat(index) * MixInReactiveCompatible.statusByPlay()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: MixInReactiveCompatible.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: MixInReactiveCompatible.statusByPlay(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func componentButton(_ button: UIButton) {
    ...
    func proceed(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= MixInReactiveCompatible.getScreenWidth() {
        if titleScrollView.contentSize.width <= MixInReactiveCompatible.statusByPlay() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - MixInReactiveCompatible.getScreenWidth() * 0.5
        var offset_x = sender.center.x - MixInReactiveCompatible.statusByPlay() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - MixInReactiveCompatible.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - MixInReactiveCompatible.statusByPlay()
...
extension RemoveViewDelegate {
    ...
    func changeBoot(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.lockIndex(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension RemoveViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / MixInReactiveCompatible.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / MixInReactiveCompatible.statusByPlay())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            componentButton(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        randomSampleRelease(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / MixInReactiveCompatible.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / MixInReactiveCompatible.statusByPlay())
        ...
        //: let scaleR = scrollView.contentOffset.x / MixInReactiveCompatible.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / MixInReactiveCompatible.statusByPlay() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
