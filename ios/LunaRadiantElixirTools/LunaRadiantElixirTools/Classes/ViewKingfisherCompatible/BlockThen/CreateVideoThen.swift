...
/*: "Finish" :*/
fileprivate let mainVideoValue:String = "f"
fileprivate let constMaxMsg:[Character] = ["i","n","i","s","h"]

/*: "Cancel" :*/
fileprivate let dataReadText:String = "Cancelreturn live guard i extension"

/*: "btn_photo_choice_pre" :*/
fileprivate let noti_nextKey:String = "btn_phshared maximum video all"
fileprivate let app_mmName:String = "oto_cempty party type for"
fileprivate let kEqualData:String = "view viewhoice_pre"

/*: "btn_photo_add_nor" :*/
fileprivate let show_clearData:String = "btn_phmake message frame data cell"
fileprivate let noti_homePillContent:[Character] = ["a"]
fileprivate let app_pathKey:String = "record click if let senddd_nor"

/*: "( :*/
fileprivate let userContentBottomViewKey:[Character] = ["("]

/*: )" :*/
fileprivate let main_simultaneouslyTitleModeUrl:[Character] = [")"]

/*: "/tmp" :*/
fileprivate let kLabelUrl:[Character] = ["/","t","m","p"]

/*: "Export session failed" :*/
fileprivate let showNextUrl:String = "self modelEx"
fileprivate let show_finishId:String = "type varport "
fileprivate let notiAllowStr:String = "title match frameion "

/*: "Export canceled" :*/
fileprivate let mainAppManagerCameraName:[Character] = ["E","x","p","o"]
fileprivate let k_errorPlusData:String = "rt cantrue equal"

/*: "Successful!" :*/
fileprivate let main_nameText:String = "Succvalue transform shared super"

/*: "Unsupported video formats" :*/
fileprivate let k_dataMsg:String = "view height if errorUnsup"
fileprivate let k_errorTitle:String = "add item class model alonged v"
fileprivate let mainSendToContent:[Character] = ["r","m","a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let notiQuoteRegularValue:String = "requestrequest"
fileprivate let notiItemContent:String = "-HH:mtitle temp else to"
fileprivate let const_fileUrl:String = "commoncommoncommon"

/*: "/tmp/video- :*/
fileprivate let noti_labelKey:String = "/tmp/vif video view"

/*: - :*/
fileprivate let kCoverStr:String = "-"

/*: .mp4" :*/
fileprivate let user_topMainKey:String = ".mp4view none"
...
class CreateVideoThen: NSObject {
    ...
    class func notary(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = appUseMessage * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = appUseMessage
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (kStatusName - height) / 2, width: appUseMessage, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (mainVideoValue.uppercased() + String(constMaxMsg)).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(dataReadText.prefix(6))).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.colorTitle(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.doTitle()
        ...
        imagePicker?.naviTitleFont = UIFont.colorTitle(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.blindConstraint()
        ...
        imagePicker?.iconThemeColor = UIColor.readModeAchromaticColour()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.barName(name: (String(noti_nextKey.prefix(6)) + String(app_mmName.prefix(5)) + String(kEqualData.suffix(9))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.barName(name: (String(show_clearData.prefix(6)) + "oto_" + String(noti_homePillContent) + String(app_pathKey.suffix(6))))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.colorTitle(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((mainVideoValue.uppercased() + String(constMaxMsg)).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((mainVideoValue.uppercased() + String(constMaxMsg)).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.readModeAchromaticColour()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: appUseMessage - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func stack(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(kLabelUrl)))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(kLabelUrl))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(showNextUrl.suffix(2)) + String(show_finishId.suffix(5)) + "sess" + String(notiAllowStr.suffix(4)) + "failed"))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(mainAppManagerCameraName) + String(k_errorPlusData.prefix(6)) + "celed"))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(main_nameText.prefix(4)) + "essful!"))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(k_dataMsg.suffix(5)) + "port" + String(k_errorTitle.suffix(4)) + "ideo fo" + String(mainSendToContent)))
            }
        }
    }
    ...
    private class func occurrent() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (notiQuoteRegularValue.replacingOccurrences(of: "request", with: "yy") + "-MM-dd" + String(notiItemContent.prefix(5)) + "m:ss-" + const_fileUrl.replacingOccurrences(of: "common", with: "S"))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(noti_labelKey.prefix(6)) + "ideo-") + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(user_topMainKey.prefix(4))))
        //: return outputPath
        return outputPath
    }
}
