...
class VideoReactiveCompatible: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: PathMixCountLiteral?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.humanHead()
            }
        }
    ...
    func momentMake() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.afterColorTarget(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.messageText()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.blindConstraint(), NSAttributedString.Key.font: UIFont.colorTitle(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.afterColorTarget(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.blindConstraint(), NSAttributedString.Key.font: UIFont.colorTitle(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.blindConstraint()
...
extension VideoReactiveCompatible: UINavigationControllerDelegate {
...
extension VideoReactiveCompatible: UIGestureRecognizerDelegate {
