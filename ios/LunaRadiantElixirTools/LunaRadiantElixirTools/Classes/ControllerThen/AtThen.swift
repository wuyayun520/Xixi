...
fileprivate let const_norFormat:[UInt8] = [0x12,0x15,0x12,0xf,0x53,0x18,0x14,0x1f,0x1e,0x9,0x41,0x52,0x5b,0x13,0x1a,0x8,0x5b,0x15,0x14,0xf,0x5b,0x19,0x1e,0x1e,0x15,0x5b,0x12,0x16,0xb,0x17,0x1e,0x16,0x1e,0x15,0xf,0x1e,0x1f]

private func removeCurve(background num: UInt8) -> UInt8 {
    return num ^ 123
}

/*: "btn_live_toast_bg" :*/
fileprivate let showImageContent:[Character] = ["b","t","n","_","l","i","v","e","_","t","o","a"]
fileprivate let data_failFormat:[Character] = ["s","t","_","b","g"]

/*: "btn_live_toast_arrow" :*/
fileprivate let app_shouldFormat:String = "btn_lmake name view"
fileprivate let k_transactionStr:String = "ive_image my log true random"
fileprivate let app_iconBoundMsg:String = "_arrowpush view section with"
...
class AtThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_norFormat.map{removeCurve(background: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        var img = UIImage.barName(name: (String(showImageContent) + String(data_failFormat))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        ...
        imgV.image = UIImage.barName(name: (String(app_shouldFormat.prefix(5)) + String(k_transactionStr.prefix(4)) + "toast" + String(app_iconBoundMsg.prefix(6))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension AtThen {
    ...
    func exhibitUponView(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard showUseImageData == false else { return }
        ...
        //: guard StatisticalTableReactiveCompatible.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard StatisticalTableReactiveCompatible.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        showUseImageData = true

        //: textLab.text = StatisticalTableReactiveCompatible.share.appUserConfigMode.liveTabTips
        textLab.text = StatisticalTableReactiveCompatible.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = appUseMessage - 48
...
extension AtThen {
    ...
    private func derivationSubviews() {
