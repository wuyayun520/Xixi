...
fileprivate let mainDataPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class BlockViewDelegate: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainDataPath.reversed(), encoding: .utf8)!)
    }

    //: func customUI() {
    func pathUi() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(mentalPicturePlay))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(drawBackTap))
...
extension BlockViewDelegate: UIScrollViewDelegate {
    ...
    @objc func mentalPicturePlay(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func drawBackTap(tap: UIGestureRecognizer) {
    ...
    func columnUrl(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.sinceMoment(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func opend(image: UIImage) {
