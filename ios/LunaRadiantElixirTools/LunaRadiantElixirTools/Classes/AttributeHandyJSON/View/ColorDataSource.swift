...
fileprivate let notiSaveMessage:[UInt8] = [0x24,0x23,0x24,0x39,0x65,0x2e,0x22,0x29,0x28,0x3f,0x77,0x64,0x6d,0x25,0x2c,0x3e,0x6d,0x23,0x22,0x39,0x6d,0x2f,0x28,0x28,0x23,0x6d,0x24,0x20,0x3d,0x21,0x28,0x20,0x28,0x23,0x39,0x28,0x29]

private func stretchIn(play num: UInt8) -> UInt8 {
    return num ^ 77
}
...
class ColorDataSource: UICollectionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiSaveMessage.map{stretchIn(play: $0)}, encoding: .utf8)!)
    }

    //: func configModel(model: TalkingMomentModel) {
    func titleTheory(model: MakeHandyJSON) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: self.reloadData()
        self.reloadData()
...
extension ColorDataSource: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, insetForSectionAt _: Int) -> UIEdgeInsets {
        ...
        if cunrrenModel.pic.count == 4 {
            ...
            return UIEdgeInsets(top: 0, left: 0, bottom: 0, right: appUseMessage - 200 - 30)
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if cunrrenModel.pic.count == 1 {
            ...
            var width = isMyhost ? (appUseMessage - 30 - 24 - 30) / 3 : (appUseMessage - 30 - 24) / 3
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection _: Int) -> CGSize {
        ...
        return CGSize(width: appUseMessage, height: 1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: UpMakePhotosCell = collectionView.dequeueReusableCell(withReuseIdentifier: UpMakePhotosCell.className(), for: indexPath) as! UpMakePhotosCell
        //: let model: TalkingMomentPhotoModel = cunrrenModel.pic[indexPath.row]
        let model: TalkingHandyJSON = cunrrenModel.pic[indexPath.row]
        //: cell.configUrl(url: model.thumbUrl ?? "")
        cell.visualCommunication(url: model.thumbUrl ?? "")
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = InsertPhotosVc(momentModel: cunrrenModel, index: indexPath.row, type: .normal)
...
extension ColorDataSource {
    ...
    private func outsideColor() {
        ...
        register(UpMakePhotosCell.self, forCellWithReuseIdentifier: UpMakePhotosCell.className())
        //: dataSource = self
        dataSource = self
        //: delegate = self
        delegate = self
