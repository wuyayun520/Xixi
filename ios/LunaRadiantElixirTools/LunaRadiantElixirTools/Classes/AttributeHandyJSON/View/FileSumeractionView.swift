...
fileprivate let app_infoTotalValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let dataImageMessage:[Character] = ["b","t","n","_","d","y","n","a","m","i","c","_","s","t","o"]
fileprivate let k_gestureLayerFromValue:[Character] = ["p","_","n","o","r"]

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let notiDataActionFormat:String = "import table name minimumbtnwO"
fileprivate let k_acceptHiddenName:String = "_tdj_share add element"
fileprivate let const_targetStr:String = "oediv_make make if"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let constLiveMessage:String = "btnCmjapp appear else action"
fileprivate let notiTitleMsg:String = "index a to pic giftoediv_"

/*: "%@" :*/
fileprivate let app_interactionStr:[Character] = ["%","@"]
...
protocol PromiseThen: NSObject {
    ...
    func weight(view: FileSumeractionView, status: AddViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func note(view: FileSumeractionView, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class FileSumeractionView: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: PromiseThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_infoTotalValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.barName(name: (String(dataImageMessage) + String(k_gestureLayerFromValue))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(senderAdd(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.barName(name: (String(notiDataActionFormat.suffix(5)) + "blEh" + String(k_acceptHiddenName.prefix(5)) + String(const_targetStr.prefix(6)) + "stop")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.barName(name: (String(constLiveMessage.prefix(6)) + "oS7_tdj_" + String(notiTitleMsg.suffix(6)) + "play")), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(imageSnapAir(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.barName(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: DateReactiveCompatible = {
        ...
        let slid = DateReactiveCompatible(frame: CGRect(x: 100, y: 0, width: appUseMessage - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension FileSumeractionView {
    ...
    private func changeList() {
        ...
        self.slider.panBeginBlock = { [weak self] in
            ...
            self.betweenModelStatus(status: .StartSeek)
        }
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.note(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] _ in
            ...
            self.betweenModelStatus(status: .StopSeek)
        }
    }
}
