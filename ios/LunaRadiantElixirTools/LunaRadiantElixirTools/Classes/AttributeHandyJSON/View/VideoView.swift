...
fileprivate let show_postModelName:[UInt8] = [0xd9,0xde,0xd9,0xc4,0x98,0xd3,0xdf,0xd4,0xd5,0xc2,0x8a,0x99,0x90,0xd8,0xd1,0xc3,0x90,0xde,0xdf,0xc4,0x90,0xd2,0xd5,0xd5,0xde,0x90,0xd9,0xdd,0xc0,0xdc,0xd5,0xdd,0xd5,0xde,0xc4,0xd5,0xd4]

private func showName(reason num: UInt8) -> UInt8 {
    return num ^ 176
}

/*: "#4FAAFF" :*/
fileprivate let main_toStr:[Character] = ["#","4","F","A","A","F","F"]

/*: "reply" :*/
fileprivate let mainViewStr:String = "ofeply"

/*: "： :*/
fileprivate let dataLanguageFormat:String = "add"

/*: "F5F5F5" :*/
fileprivate let k_labelName:[Character] = ["F","5","F","5","F","5"]
...
class VideoView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_postModelName.map{showName(reason: $0)}, encoding: .utf8)!)
    }
...
extension VideoView {
    ...
    @objc func handle(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension VideoView {
    //: func configCell(model: TalkingCommentReplyModel) {
    func sumerval(model: AttributeHandyJSON) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(main_toStr)))
            ...
            let font = UIFont.statusDataMove(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (mainViewStr.replacingOccurrences(of: "of", with: "r")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.shouldHiddenAppear(), UIColor.blindConstraint(), UIColor.shouldHiddenAppear(), UIColor.blindConstraint()]
                ...
                colorA = [UIColor.shouldHiddenAppear(), UIColor.blindConstraint()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func beforeAction(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: appUseMessage - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (String(k_labelName)))?.cgColor
...
extension VideoView {
    ...
    private func sinceNow() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(handle(sender:)))
    ...
    private func subTableConstraint() {
