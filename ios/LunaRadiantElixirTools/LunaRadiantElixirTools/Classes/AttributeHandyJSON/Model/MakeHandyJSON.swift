...
/*: "id" :*/
fileprivate let const_sharedLevelValue:[Character] = ["i","d"]
...
struct TopMeasurable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: VideoMakeExpressionConvertible?
}

///
//: enum MomentVideoStatus: String {
enum VideoMakeExpressionConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension MakeHandyJSON {
    ...
    mutating func search(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(ImageReactiveCompatible.beyondTo(text: content!, size: CGSize(width: appUseMessage - 54, height: CGFloat(MAXFLOAT)), font: UIFont.colorTitle(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MixMirrorPath.Photo.rawValue || momentType == MixMirrorPath.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
