...
fileprivate let constRequestValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Save successfully" :*/
fileprivate let noti_addTitle:String = "block moment infoSave "
fileprivate let app_tempStr:String = "sbottomcc"
fileprivate let data_leadingMsg:String = "etimetimef"

/*: / :*/
fileprivate let mainColorTitle:String = "path"
...
class InsertPhotosVc: ItemTopViewController {
    ...
    init(momentModel: MakeHandyJSON, index: Int, type: FillPhotoType) {
        ...
        if CapacityLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(appUseMessage), y: 0)
    ...
    init(images: [UIImage], index: Int, type: FillPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constRequestValue.reversed(), encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: appUseMessage, height: kStatusName)
        ...
        view.frame = CGRect(x: 0, y: 0, width: appUseMessage, height: kStatusName)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(appUseMessage), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(appUseMessage), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(appUseMessage), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(BlockViewDelegate.self, forCellWithReuseIdentifier: BlockViewDelegate.className())
...
//    lazy var downloadBtn: TalkingItemButton = {
//        let btn = TalkingItemButton.init()
...
//        UpPermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: TalkingHandyJSON = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.taproom(showMsg: (String(noti_addTitle.suffix(5)) + app_tempStr.replacingOccurrences(of: "bottom", with: "u") + data_leadingMsg.replacingOccurrences(of: "time", with: "s") + "ully").localized)
                }
                //: } else {
            } else {
...
extension InsertPhotosVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func enableClick() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func pillSample() {
        //: if  currenModel.sex==StatisticalTableReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == StatisticalTableReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: BlockViewDelegate = collectionView.dequeueReusableCell(withReuseIdentifier: BlockViewDelegate.className(), for: indexPath) as! BlockViewDelegate
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: TalkingHandyJSON = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.columnUrl(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.opend(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.opend(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: BlockViewDelegate = cell as! BlockViewDelegate
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / appUseMessage)
        //: if myType == .normal {
        if myType == .normal {
