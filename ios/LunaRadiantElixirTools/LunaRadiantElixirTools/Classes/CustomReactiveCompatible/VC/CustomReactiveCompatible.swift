...
class CustomReactiveCompatible: DropBaseViewController {
    //: var type = LoginType.PhoneLogin
    var type = TypeDefaultsSerializable.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: ModeFeedPwdView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = ModeFeedPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension CustomReactiveCompatible {
    ...
    private func nameManager(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        MixItemReactiveCompatible.premium(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    private func available() {
        ...
        MixItemReactiveCompatible.enterAgeCompletion { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            BeanNameProgressHUD.doingBlock()
            //: guard let self = self else { return }
            guard let self = self else { return }
