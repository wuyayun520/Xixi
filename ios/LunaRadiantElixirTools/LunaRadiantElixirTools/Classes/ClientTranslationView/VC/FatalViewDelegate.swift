...
/*: "New friends" :*/
fileprivate let showMakeText:String = "image self image view cookieNew "
fileprivate let const_toName:String = "nviews"

/*: "icon_yidu_pre" :*/
fileprivate let notiActionMsg:String = "play add background commonicon_yi"
fileprivate let constKeyStageFormat:String = "du_prefor color top kit"

/*: "You've got no message yet." :*/
fileprivate let data_requestTitleMsg:String = "You\'v"
fileprivate let app_topMessage:String = "view view make to selfo mess"

/*: "icon_kong_kong_default" :*/
fileprivate let noti_equalKey:String = "with cellicon_"
fileprivate let const_labelMessage:String = "_kontime self select let path"
fileprivate let const_topId:String = "icon content section imageg_default"

/*: "Cancel" :*/
fileprivate let k_colorName:String = "Cancescreen hidden will let"
fileprivate let k_partyFormat:[Character] = ["l"]

/*: "OK" :*/
fileprivate let userJoinMsg:String = "Osucceed"

/*: "uid" :*/
fileprivate let appAdjustData:[UInt8] = [0x34,0x28,0x25]

/*: "Do you want to mark all messages as read?" :*/
fileprivate let kShowText:[UInt8] = [0x28,0x3,0x4c,0x15,0x3,0x19,0x4c,0x1b,0xd,0x2,0x18,0x4c,0x18,0x3,0x4c,0x1,0xd,0x1e,0x7,0x4c,0xd,0x0,0x0,0x4c,0x1,0x9,0x1f,0x1f,0xd,0xb,0x9,0x1f,0x4c,0xd,0x1f,0x4c,0x1e,0x9,0xd,0x8,0x53]

private func labelLet(size num: UInt8) -> UInt8 {
    return num ^ 108
}

/*: "MoreMessages一键已读失败：code: :*/
fileprivate let app_indexText:String = "Moretrue back"
fileprivate let main_giftPath:String = "send any head let aages一键"
fileprivate let user_frameTitle:String = "title"
fileprivate let show_countName:String = "读失败："

/*: , desc: :*/
fileprivate let showSumervalData:[Character] = [","," ","d","e","s","c",":"]

/*: "btn_message_report_nor" :*/
fileprivate let k_userName:[Character] = ["b","t","n","_","m"]
fileprivate let noti_recordLabelMsg:[Character] = ["e","s"]
fileprivate let kLabEmptyHiddenData:String = "sagcontent"
fileprivate let kFinishTablePath:String = "time self plus data valuert_nor"

/*: "#FF935D" :*/
fileprivate let app_byEqualMessage:String = "#FF935Dyear of view make app"

/*: "btn_message_block_nor" :*/
fileprivate let notiContextMsg:String = "path from frame mybtn_m"
fileprivate let app_shareVideoMsg:String = "user any stringge_b"
fileprivate let noti_pathData:String = "lostartk"

/*: "#C179F9" :*/
fileprivate let const_multipleUrl:[Character] = ["#","C","1","7","9","F","9"]

/*: "btn_message_delete_nor" :*/
fileprivate let constMaleCheckPath:String = "color top rangebtn_"
fileprivate let noti_maxId:String = "ge_din stop else"

/*: "#FF506D" :*/
fileprivate let showCountKey:String = "#FF50feature top self to"
fileprivate let show_whiteFormat:String = "6Dof gesture type"
...
class FatalViewDelegate: DropBaseViewController {
    //: override func viewDidLoad() {
    override func viewDidLoad() {
        //: super.viewDidLoad()
        super.viewDidLoad()
        //: self.title = "New friends".localized
        self.title = (String(showMakeText.suffix(4)) + "frie" + const_toName.replacingOccurrences(of: "view", with: "d")).localized
        ...
        btn.setImage(UIImage.barName(name: (String(notiActionMsg.suffix(7)) + String(constKeyStageFormat.prefix(6)))), for: .normal)
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (data_requestTitleMsg + "e got n" + String(app_topMessage.suffix(6)) + "age yet.").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(noti_equalKey.suffix(5)) + "kong" + String(const_labelMessage.prefix(4)) + String(const_topId.suffix(9)))
...
extension FatalViewDelegate {
    ...
    func packageAcross(conversationModel: ResultInfoConversationModel) {
        ...
        //: DrawMsgListener.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: .moreList)
        DrawMsgListener.shared.targetType(targetId: conversationModel.targetId, dataType: .moreList)
    ...
    func chromaticColor(conversationModel: ResultInfoConversationModel) {
        // 拉黑/解除拉黑
        ...
        BlockThen.rangeConfig(title: nil,
                                     ...
                                     leftBtnTitle: (String(k_colorName.prefix(5)) + String(k_partyFormat)).localized,
                                     ...
                                     rightBtnTitle: "OK".localized)
        {
            ...
            UserFirstReactiveCompatible.bindEqual(uid: conversationModel.userID, isBlack: true) { succeed, _, _ in
                ...
                self.packageAcross(conversationModel: conversationModel)
                //: NotificationCenter.default.post(name: LIVE_BLOCK_USER_NOTIFICATION,
                NotificationCenter.default.post(name: const_clickId,
                                                //: object: nil,
                                                object: nil,
                                                //: userInfo: ["uid": conversationModel.userID])
                                                userInfo: [String(bytes: appAdjustData.map{$0^65}, encoding: .utf8)!: conversationModel.userID])
            }
        }
    }
    ...
    func streetwiseUp(conversationModel: ResultInfoConversationModel) {
    ...
    func getString() -> String {
        return String(bytes: kShowText.map{labelLet(size: $0)}, encoding: .utf8)!.localized
    }
    ...
    @objc private func argument() {
        ...
        BlockThen.collectionConfig(message: self.getString(), leftBtnTitle: (String(k_colorName.prefix(5)) + String(k_partyFormat)).localized, rightBtnTitle: "OK".localized, leftBlock: {
            //: TalkingAlertShow.hideAlert()
            BlockThen.buttonIn()
            ...
            guard self.manager.anRe() > 0 else { return }
...
extension FatalViewDelegate {
    ...
    func netPutIn() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(viewHire(notification:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(warpathNotif(notif:)),
    ...
    @objc func viewHire(notification: NSNotification) {
    ...
    @objc private func warpathNotif(notif: Notification) {
...
extension FatalViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var model = self.manager.track(indexPath: indexPath)
    ...
    func tableView(_: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        ...
        guard let model = self.manager.track(indexPath: indexPath) else {
            //: return false
            return false
        }
    ...
    func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        guard let model = self.manager.track(indexPath: indexPath) else { return nil }
        ...
        let reportBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.streetwiseUp(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let reportBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_report_nor").cgImage {
        if let reportBtnTrashcan = UIImage.barName(name: (String(k_userName) + String(noti_recordLabelMsg) + kLabEmptyHiddenData.replacingOccurrences(of: "content", with: "e") + "_repo" + String(kFinishTablePath.suffix(6)))).cgImage {
        ...
        reportBtn.backgroundColor = UIColor(hex: (String(app_byEqualMessage.prefix(7))))
        ...
        let blockBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.chromaticColor(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let blockBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_block_nor").cgImage {
        if let blockBtnTrashcan = UIImage.barName(name: (String(notiContextMsg.suffix(5)) + "essa" + String(app_shareVideoMsg.suffix(4)) + noti_pathData.replacingOccurrences(of: "start", with: "c") + "_nor")).cgImage {
        ...
        blockBtn.backgroundColor = UIColor(hex: (String(const_multipleUrl)))
        ...
        let deleteBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            ...
            self.packageAcross(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let deleteBtnTrashcan = UIImage.BundleImageNamed(name: "btn_message_delete_nor").cgImage {
        if let deleteBtnTrashcan = UIImage.barName(name: (String(constMaleCheckPath.suffix(4)) + "messa" + String(noti_maxId.prefix(4)) + "elete_nor")).cgImage {
        ...
        deleteBtn.backgroundColor = UIColor(hex: (String(showCountKey.prefix(5)) + String(show_whiteFormat.prefix(2))))
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //: EditPushManager.share.func__pushToPriveteChatVC(chatID: model.targetId) { vc in
        EditPushManager.share.fromDataConverterMomentTipCompletionPushChatPrivete(chatID: model.targetId) { vc in
...
extension FatalViewDelegate: DelegateObjectProtocol {
    //: func onRefreshConversationList(data: [TalkingConversationModel]) {
    func towardPosition(data _: [ResultInfoConversationModel]) {
        //: self.manager.req_moreMsgInitData()
        self.manager.arc()
        //: refreshTableView()
        anyEqual()
