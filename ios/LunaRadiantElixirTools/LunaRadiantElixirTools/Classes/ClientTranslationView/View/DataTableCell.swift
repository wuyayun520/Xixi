...
fileprivate let dataAccountName:[UInt8] = [0x87,0x80,0x87,0x9a,0xc6,0x8d,0x81,0x8a,0x8b,0x9c,0xd4,0xc7,0xce,0x86,0x8f,0x9d,0xce,0x80,0x81,0x9a,0xce,0x8c,0x8b,0x8b,0x80,0xce,0x87,0x83,0x9e,0x82,0x8b,0x83,0x8b,0x80,0x9a,0x8b,0x8a]

private func openList(lab num: UInt8) -> UInt8 {
    return num ^ 238
}

/*: "#30D00B" :*/
fileprivate let mainViewImageUrl:[Character] = ["#","3","0","D","0"]
fileprivate let show_pickKey:[Character] = ["0","B"]

/*: "icon_receivedcalls" :*/
fileprivate let constJoinName:String = "icposition"
fileprivate let notiEditMsg:String = "edcatexttexts"

/*: "Received calls" :*/
fileprivate let notiAddData:String = "let let track hidden pointRece"
fileprivate let show_quoteViewPath:String = "table"

/*: "icon_dialedcalls" :*/
fileprivate let noti_appTargetTitle:[Character] = ["i","c","o","n","_","d","i"]
fileprivate let k_viewerData:String = "alsectionc"

/*: "Dialed calls" :*/
fileprivate let k_deviceFieldMessage:String = "add message guard typeDiale"
fileprivate let constQueryedPath:[Character] = ["s"]

/*: "%02i:%02i" :*/
fileprivate let app_fromValue:String = "resign content%02i:%02"
fileprivate let userLeadingSearchText:String = "size"
...
class DataTableCell: UITableViewCell {
    ...
    var currenModel = MajorMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataAccountName.map{openList(lab: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(viewKey), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .colorTitle(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .shouldHiddenAppear()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(mainViewImageUrl) + String(show_pickKey)))
...
extension DataTableCell {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func workIn(model: MajorMeasurable) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.cramFor(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.barName(name: (constJoinName.replacingOccurrences(of: "position", with: "on") + "_receiv" + notiEditMsg.replacingOccurrences(of: "text", with: "l")))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(notiAddData.suffix(4)) + "ived call" + show_quoteViewPath.replacingOccurrences(of: "table", with: "s")).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.barName(name: (String(noti_appTargetTitle) + k_viewerData.replacingOccurrences(of: "section", with: "ed") + "alls"))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(k_deviceFieldMessage.suffix(5)) + "d call" + String(constQueryedPath)).localized
        }
