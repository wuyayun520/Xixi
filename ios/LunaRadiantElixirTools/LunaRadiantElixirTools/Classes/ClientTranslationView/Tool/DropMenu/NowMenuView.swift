...
fileprivate let kDataBindKey:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

private func playerIn(action num: UInt8) -> UInt8 {
    return num ^ 118
}

/*: "SHBDropMenuCell" :*/
fileprivate let kClickFormat:[Character] = ["S","H","B","D","r","o","p","M","e"]
fileprivate let kInfoMessageData:String = "nuCelllabel var by advertising"

/*: "title" :*/
fileprivate let user_circleMessage:String = "TITLE"
...
public class NowMenuView: UIView {
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.MonthPromise.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.FileBlock.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.FileBlock.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.FileBlock.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? EditMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kDataBindKey.map{playerIn(action: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func headache() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func currentView() -> UITableView {
    ...
    private func messageView() {
        ...
        mTable.register(CeremonyViewCell.self, forCellReuseIdentifier: (String(kClickFormat) + String(kInfoMessageData.prefix(6))))
    ...
    private func fileWidth() -> CGFloat {
    ...
    private func rangeContentTriangle() {
...
extension NowMenuView: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(kClickFormat) + String(kInfoMessageData.prefix(6))), for: indexPath) as! CeremonyViewCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (user_circleMessage.lowercased()))
        //: animateRemoveView()
        discharge()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.rowGift(index: indexPath.row, title: cellModel.title ?? "")
    }
}
