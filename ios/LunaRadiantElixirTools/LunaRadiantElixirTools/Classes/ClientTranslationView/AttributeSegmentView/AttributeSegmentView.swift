...
fileprivate let data_frameTitle:String = "设置普通状态"
fileprivate let dataLivePushStr:String = "时 请使用RGof any record"
fileprivate let app_tableText:String = "B空间的颜色值lab user"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let data_labMsg:[Character] = ["设","置","选","中","状","态","的","\u{6587}","\u{5b57}","颜","色","时"]
fileprivate let noti_makePath:[Character] = [" ","请","使","用","R","G","B"]
fileprivate let user_modelStr:[Character] = ["\u{7a7a}","间","的","\u{989c}","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let mainManagerStr:[UInt8] = [0xe2,0xe5,0xe2,0xff,0xa3,0xe8,0xe4,0xef,0xee,0xf9,0xb1,0xa2,0xab,0xe3,0xea,0xf8,0xab,0xe5,0xe4,0xff,0xab,0xe9,0xee,0xee,0xe5,0xab,0xe2,0xe6,0xfb,0xe7,0xee,0xe6,0xee,0xe5,0xff,0xee,0xef]

/*: "设置的下标不合法!!" :*/
fileprivate let showVerticalTitle:String = "设置的\u{4e0b}"

/*: "Bags" :*/
fileprivate let const_viewHalfTitle:String = "video count style self makeBags"
...
open class AttributeSegmentView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: RawSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.behindAppConvert(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((data_frameTitle.capitalized + "的文字颜色" + String(dataLivePushStr.prefix(7)) + String(app_tableText.prefix(7))))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.behindAppConvert(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(data_labMsg) + String(noti_makePath) + String(user_modelStr)))
        }
    ...
    fileprivate func behindAppConvert(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: RawSegmentStyle, titles: [String]) {
    ...
    @objc func liveFileClick(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? TravelKitThen else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainManagerStr.map{$0^139}, encoding: .utf8)!)
    }
}
...
public extension AttributeSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func encephalon(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (showVerticalTitle + "标不合\u{6cd5}!!"))
    ...
    func calefactory(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension AttributeSegmentView {
    // 根据Titles添加相应的控件
    ...
    fileprivate func ok() {
        ...
        for (index, title) in titles.enumerated() {
            ...
            let label = TravelKitThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.liveFileClick(_:)))
            ...
            if title == (String(const_viewHalfTitle.suffix(4))).localized {
    ...
    fileprivate func positionLive() {
        ...
        if let firstLabel = labelsArray[0] as? TravelKitThen {
    ...
    fileprivate func priceByActive() {
    ...
    func noCell(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: (String(const_viewHalfTitle.suffix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            TableUpReactiveCompatible.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! TravelKitThen
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! TravelKitThen
        // 让选中标签居中显示
        //: adjustTitleOffSetToCurrentIndex(currentIndex)
        separate(currentIndex)
        // 动画效果
    ...
    public func separate(_ currentIndex: Int) {
...
open class TravelKitThen: UILabel {
    /// 用来记录当前label的缩放比例
