...
fileprivate let user_mediumId:[UInt8] = [0x76,0x71,0x76,0x6b,0x37,0x7c,0x70,0x7b,0x7a,0x6d,0x25,0x36,0x3f,0x77,0x7e,0x6c,0x3f,0x71,0x70,0x6b,0x3f,0x7d,0x7a,0x7a,0x71,0x3f,0x76,0x72,0x6f,0x73,0x7a,0x72,0x7a,0x71,0x6b,0x7a,0x7b]

private func viewMake(detail num: UInt8) -> UInt8 {
    return num ^ 31
}

/*: "Reply" :*/
fileprivate let main_colorFormat:String = "gift let private selectedReply"

/*: "#F95151" :*/
fileprivate let notiSaveStr:[Character] = ["#","F","9","5","1","5","1"]
...
let showRecordKey = (user_keyId + 80.0)
//: class TalkingMessageNotificationView: UIView {
class AlongNotificationView: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: ResultInfoConversationModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: AlongNotificationView) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = ResultInfoConversationModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_mediumId.map{viewMake(detail: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .colorTitle(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .colorTitle(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(main_colorFormat.suffix(5))).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(notiSaveStr))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension AlongNotificationView {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func keyByShow(_ model: ResultInfoConversationModel?) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    ...
                    self.doingLive(needAlpha: false)
                }
            }
        }
    }
    ...
    func doingLive(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -showRecordKey
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func motion() {
        ...
        if currModel.chatType == .service {
            ...
            //: EditPushManager.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            EditPushManager.share.fromDataConverterMomentTipCompletionPushChatPrivete(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func occurrentBy(gesture: UISwipeGestureRecognizer) {
...
extension AlongNotificationView {
    ...
    func textImage() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.mentalImageDownRelease(urlStr: userInfo.headPic, placeImg: UIImage.targetSex(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.cramFor(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
    ...
    private func route() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(motion))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(occurrentBy))
        //: swipe.direction = .up
        swipe.direction = .up
