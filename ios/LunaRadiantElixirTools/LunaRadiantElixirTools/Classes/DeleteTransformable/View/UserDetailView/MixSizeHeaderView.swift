...
fileprivate let user_overColorValue:[UInt8] = [0x70,0x77,0x70,0x6d,0x31,0x7a,0x76,0x7d,0x7c,0x6b,0x23,0x30,0x39,0x71,0x78,0x6a,0x39,0x77,0x76,0x6d,0x39,0x7b,0x7c,0x7c,0x77,0x39,0x70,0x74,0x69,0x75,0x7c,0x74,0x7c,0x77,0x6d,0x7c,0x7d]

private func trackData(shared num: UInt8) -> UInt8 {
    return num ^ 25
}

/*: "icon_userdetail_header_shadow" :*/
fileprivate let show_cloudData:String = "useron"
fileprivate let kTextMakeValue:String = "rdetimage"
fileprivate let userQuoteData:String = "white let up toder_s"

/*: "icon_lounge" :*/
fileprivate let showOkCellData:String = "let in button let qualityicon_"
fileprivate let const_targetMsg:String = "loscreenge"

/*: "Official" :*/
fileprivate let noti_windowMessage:String = "Officiamodel if string"
fileprivate let k_indexStr:String = "image"

/*: "Reviewing" :*/
fileprivate let user_senseStr:String = "Revilet manager var self frame"

/*: "#FF506D" :*/
fileprivate let dataDirectionPath:[Character] = ["#","F","F","5","0","6","D"]

/*: "icon_user_detail_intimate_header" :*/
fileprivate let app_labelKey:[UInt8] = [0x76,0x70,0x7c,0x7b,0x6c,0x82,0x80,0x72,0x7f,0x6c,0x71,0x72,0x81,0x6e,0x76,0x79,0x6c,0x76,0x7b,0x81,0x76,0x7a,0x6e,0x81,0x72,0x6c,0x75,0x72,0x6e,0x71,0x72,0x7f]

fileprivate func roundEmptySumeraction(make num: UInt8) -> UInt8 {
...
/*: "?toUid= :*/
fileprivate let const_toValue:[Character] = ["?","t","o","U","i","d","="]

/*: "ItemThen" :*/
fileprivate let k_frameMessage:String = "model to returnTUserD"
fileprivate let userAddFormat:String = "lHeaview gift bag"
fileprivate let notiImageMessage:String = "along add allemCell"

/*: "#FFFFFF" :*/
fileprivate let main_editMessage:String = "table play should data#FFFFF"
fileprivate let noti_floorValue:String = "cancel"

/*: "#BDB8B5" :*/
fileprivate let data_withMessage:[Character] = ["#","B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let user_tabText:String = "head_make self content leading"
fileprivate let appModeStr:String = "color image_botto"
fileprivate let data_barMsg:String = "video"

/*: "#2DF2FF" :*/
fileprivate let showCurrentPointFormat:String = "#2DF2FFfunction at height"

/*: "Live" :*/
fileprivate let noti_tipStr:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let mainEndUrl:String = "#48FFbe component make"
fileprivate let show_labelFormat:String = "1Eimage name"

/*: "Online" :*/
fileprivate let constCookieUrl:String = "gift"
fileprivate let userAllMakeFormat:String = "nlinkey"

/*: "#FF4B7F" :*/
fileprivate let showSolarKey:String = "#FF4B7Fselect sure"

/*: "busy" :*/
fileprivate let appThenFormat:String = "BUSY"
...
let k_netMessage = (302.0 + user_keyId)

//: class TalkingUserDetailHeaderView: UIView {
class MixSizeHeaderView: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = PathUserTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_overColorValue.map{trackData(shared: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        let frame = CGRect(x: 0, y: 0, width: appUseMessage, height: k_netMessage)
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.barName(name: (show_cloudData.replacingOccurrences(of: "user", with: "ic") + "_use" + kTextMakeValue.replacingOccurrences(of: "image", with: "a") + "il_hea" + String(userQuoteData.suffix(5)) + "hadow")))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: k_netMessage - 15, width: appUseMessage, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.font = UIFont.colorTitle(type: .Medium, fontSize: 19)
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.barName(name: (String(showOkCellData.suffix(5)) + const_targetMsg.replacingOccurrences(of: "screen", with: "un")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.mastheadState(color: UIColor.makeMessage(), forState: .normal)
        //: img.setTitle("Official".localized, for: .normal)
        img.setTitle((String(noti_windowMessage.prefix(7)) + k_indexStr.replacingOccurrences(of: "image", with: "l")).localized, for: .normal)
        //: img.setTitleColor(.white, for: .normal)
        img.setTitleColor(.white, for: .normal)
        //: img.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 11)
        img.titleLabel?.font = UIFont.statusDataMove(fontSize: 11)
    ...
    private lazy var onlineImgV: TalkingItemButton = {
    ...
    private lazy var liveView: ScreenView = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: EditPushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            EditPushManager.share.leftLuggageDoingice(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(user_senseStr.prefix(4)) + "ewing").localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(dataDirectionPath)))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .colorTitle(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: TalkingItemButton = {
        //: let btn = TalkingButton()
        let btn = TalkingItemButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.barName(name: String(bytes: app_labelKey.map{roundEmptySumeraction(make: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(frameTo), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension MixSizeHeaderView {
    /// 查看视频
    //: private func goToVideo() {
    private func totaleraction() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = PresentationVideoVc(videoPath: self.userInfoModel.videoUrl)
        //: if StatisticalTableReactiveCompatible.share.loginUid != self.userInfoModel.uid {
        if StatisticalTableReactiveCompatible.share.loginUid != self.userInfoModel.uid {
    ...
    private func beyondPhoto(_ index: Int) {
        ...
        let vc = InsertPhotosVc(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    @objc func frameTo() {
        ...
        let url = VideoManager.towardView(urlType: .Intimate) + (String(const_toValue)) + "\(self.userInfoModel.uid)"
...
extension MixSizeHeaderView: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: ItemThen.self, reuseIdentifier: (String(k_frameMessage.suffix(6)) + "etai" + String(userAddFormat.prefix(4)) + "derIt" + String(notiImageMessage.suffix(6))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && userInfoModel.sex == FalloutCompatibleValue.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.stand()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.inundate(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.mentalImageDownRelease(urlStr: urlStr, placeImg: UIImage.wantLargeMouthed())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == StatisticalTableReactiveCompatible.share.loginUid else {
        guard userInfoModel.uid == StatisticalTableReactiveCompatible.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(main_editMessage.suffix(6)) + noti_floorValue.replacingOccurrences(of: "cancel", with: "F")))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(data_withMessage)))!
...
extension MixSizeHeaderView: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == FalloutCompatibleValue.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            totaleraction()
            ...
            if userInfoModel.sex == FalloutCompatibleValue.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension MixSizeHeaderView {
    ...
    func taskView(_ userModel: PathUserTransformable) {
        ...
        if userModel.sex == FalloutCompatibleValue.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.barName(name: (String(user_tabText.prefix(5)) + "detail" + String(appModeStr.suffix(6)) + data_barMsg.replacingOccurrences(of: "video", with: "m"))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = StatisticalTableReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = StatisticalTableReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.mentalImageDownRelease(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .dataConverterColor() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.sinceMoment(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.atColorSize(color: UIColor(hex: (String(showCurrentPointFormat.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.ageEffect(), for: .normal)
            onlineImgV.setImage(image.ageEffect(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(noti_tipStr)).localized, for: .normal)
            ...
            let image = UIImage.atColorSize(color: UIColor(hex: (String(mainEndUrl.prefix(5)) + String(show_labelFormat.prefix(2))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.ageEffect(), for: .normal)
            onlineImgV.setImage(image.ageEffect(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((constCookieUrl.replacingOccurrences(of: "gift", with: "O") + userAllMakeFormat.replacingOccurrences(of: "key", with: "e")).localized, for: .normal)
            ...
            let image = UIImage.atColorSize(color: UIColor(hex: (String(showSolarKey.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.ageEffect(), for: .normal)
            onlineImgV.setImage(image.ageEffect(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((appThenFormat.lowercased()).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if StatisticalTableReactiveCompatible.share.loginUid != userModel.uid &&
        if StatisticalTableReactiveCompatible.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.cohere(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func popConstraint() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(user_keyId + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(user_keyId + 15)
