...
fileprivate let constArrayKey:[UInt8] = [0x5a,0x5d,0x5a,0x47,0x1b,0x50,0x5c,0x57,0x56,0x41,0x9,0x1a,0x13,0x5b,0x52,0x40,0x13,0x5d,0x5c,0x47,0x13,0x51,0x56,0x56,0x5d,0x13,0x5a,0x5e,0x43,0x5f,0x56,0x5e,0x56,0x5d,0x47,0x56,0x57]

/*: "icon_male_default" :*/
fileprivate let noti_colorMessage:[UInt8] = [0xc9,0xc3,0xcf,0xce,0xbf,0xcd,0xc1,0xcc,0xc5,0xbf,0xc4,0xc5,0xc6,0xc1,0xd5,0xcc,0xd4]

fileprivate func stopTo(lab num: UInt8) -> UInt8 {
...
/*: "icon_female_default" :*/
fileprivate let app_itemData:String = "momenton"
fileprivate let user_rangeName:String = "_femalafter pure"

/*: "level_%@" :*/
fileprivate let userFrameBarKey:[Character] = ["l","e","v","e","l","_","%"]
fileprivate let notiProgressFormat:String = "make"
...
class EndUserView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constArrayKey.map{$0^51}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .colorTitle(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .colorTitle(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension EndUserView {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func less(model: StochasticProcessInfoModel) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.sizeTake())
        ...
        let imgStr = model.sex == FalloutCompatibleValue.male.rawValue ? String(bytes: noti_colorMessage.map{stopTo(lab: $0)}, encoding: .utf8)! : (app_itemData.replacingOccurrences(of: "moment", with: "ic") + String(user_rangeName.prefix(6)) + "e_default")
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(userFrameBarKey) + notiProgressFormat.replacingOccurrences(of: "make", with: "@")), model.level)
