...
/*: "#AAAAAA" :*/
fileprivate let mainActionScaleData:String = "#AAAAAlet at let"
fileprivate let constLeadingMessage:String = "hidden"

/*: " No more than 10 characters" :*/
fileprivate let main_onMsg:String = "guard transform No mo"
fileprivate let constMakeValue:String = "value model burn selfan 10 "
fileprivate let main_scaleKey:String = "actimage"
fileprivate let app_inputName:[Character] = ["r","s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let noti_borderText:[UInt8] = [0xb,0xc,0xb,0x16,0x4a,0x1,0xd,0x6,0x7,0x10,0x58,0x4b,0x42,0xa,0x3,0x11,0x42,0xc,0xd,0x16,0x42,0x0,0x7,0x7,0xc,0x42,0xb,0xf,0x12,0xe,0x7,0xf,0x7,0xc,0x16,0x7,0x6]

private func pointJoin(bottom num: UInt8) -> UInt8 {
    return num ^ 98
}
...
class InfoView: UIView {
    ...
    private var alertConfig: DigitizerAlertConfig
    ...
    private lazy var inputTView: EffectScrollView = {
        ...
        input.placeholderColor = .init(hex: (String(mainActionScaleData.prefix(6)) + constLeadingMessage.replacingOccurrences(of: "hidden", with: "A"))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(main_onMsg.suffix(6)) + "re th" + String(constMakeValue.suffix(6)) + "char" + main_scaleKey.replacingOccurrences(of: "image", with: "e") + String(app_inputName)).localized
        ...
        input.backgroundColor = .messageText()
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.readModeAchromaticColour().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(force), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.atArray(colors: UIColor.govern(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.atArray(colors: UIColor.countervalMain(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(labSharedLoad), for: .touchUpInside)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_borderText.map{pointJoin(bottom: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func quantity(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.barName(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.barName(name: alertConfig.rightBackgroundImage).withTintColor(.readModeAchromaticColour())
...
extension InfoView: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func matterWithinLinemenName(_ textField: UITextField, shouldChangeCharactersIn _: NSRange, replacementString string: String) -> Bool {
    ...
    func tablePicDelete(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = chessPlayer(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func chessPlayer(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
