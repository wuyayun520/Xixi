...
fileprivate let constAttributeContent:[UInt8] = [0xfa,0xff,0xfa,0x5,0xb9,0xf4,0x0,0xf5,0xf6,0x3,0xcb,0xba,0xb1,0xf9,0xf2,0x4,0xb1,0xff,0x0,0x5,0xb1,0xf3,0xf6,0xf6,0xff,0xb1,0xfa,0xfe,0x1,0xfd,0xf6,0xfe,0xf6,0xff,0x5,0xf6,0xf5]

fileprivate func errorCell(name num: UInt8) -> UInt8 {
...
/*: "Post" :*/
fileprivate let show_tempMsg:String = "change model jawPost"

/*: "Profile" :*/
fileprivate let mainMakeMessage:String = "Profilemodel let equal source text"

/*: "bth_me_follow_nor" :*/
fileprivate let showCleanTitle:String = "content error addbth_me"
fileprivate let const_coverTitle:[Character] = ["_","f","o","l","l","o","w","_","n","o","r"]

/*: "party_mini_bg_userDetail" :*/
fileprivate let kSizeMsg:String = "partcookie"
fileprivate let user_equalMinText:String = "count file leadingi_bg_u"

/*: "removeAttentionUid" :*/
fileprivate let app_giftValue:String = "remtool"
fileprivate let user_goldContent:String = "value name equalntionU"
fileprivate let show_orientationId:[Character] = ["i","d"]

/*: "attentionUid" :*/
fileprivate let showAngleId:String = "ATTE"
fileprivate let dataDropName:String = "iframe"

/*: "bth_me_follow_pre" :*/
fileprivate let showDataFormat:[UInt8] = [0x39,0x2f,0x33,0x4,0x36,0x3e,0x4,0x3d,0x34,0x37,0x37,0x34,0x2c,0x4,0x2b,0x29,0x3e]

private func marginOfSafety(interest num: UInt8) -> UInt8 {
    return num ^ 91
}
...
class KeyInputThen: DropBaseViewController {
    //: var uid: String = ""
    var uid: String = ""
    ...
    private var giftsArr = [TodayHandyJSON]() // 收到的礼物
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constAttributeContent.map{errorCell(name: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: RefreshReactiveCompatible = { // 导航
        //: let v = TalkingUserDetailNaviBarView()
        let v = RefreshReactiveCompatible()
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var headerView: MixSizeHeaderView = { // banner
        ...
        let v = MixSizeHeaderView(frame: frame)
    ...
    private lazy var bottomView: BlockReactiveCompatible = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = BlockReactiveCompatible(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: user_keyId, width: appUseMessage, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.shouldHiddenAppear()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.blindConstraint()
        ...
        indicator.indicatorColor = UIColor.blindConstraint()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(user_viewSumervalKey)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(show_tempMsg.suffix(4))).localized, (String(mainMakeMessage.prefix(7))).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [DropBaseViewController] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: WithFlowLayout = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = WithFlowLayout()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: GroupObjectProtocol = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = GroupObjectProtocol()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.barName(name: (String(showCleanTitle.suffix(6)) + String(const_coverTitle)))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(percentageBind))
    ...
    private lazy var voiceRoomBtn: UIButton = {
        ...
        voiceRoomBtn.setBackgroundImage(UIImage.barName(name: (kSizeMsg.replacingOccurrences(of: "cookie", with: "y") + "_min" + String(user_equalMinText.suffix(6)) + "serDetail")), for: .normal)
        //: return voiceRoomBtn
        return voiceRoomBtn
        //: }()
    }()
...
extension KeyInputThen {
    ...
    private func execute() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(app_giftValue.replacingOccurrences(of: "tool", with: "o") + "veAtte" + String(user_goldContent.suffix(6)) + String(show_orientationId))] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(showAngleId.lowercased() + "ntionU" + dataDropName.replacingOccurrences(of: "frame", with: "d"))] = userInfoModel.uid
        }
    ...
    func create() {
        ...
        queue.async(group: group) {
            ...
            UserFirstReactiveCompatible.infoAwake(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.report()
                //: if self.uid == StatisticalTableReactiveCompatible.share.loginUid {
                if self.uid == StatisticalTableReactiveCompatible.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.thatHide()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<PathUserTransformable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.card(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.taskView(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.comeInView(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.coequal(userModel)
                        ...
                        self.roomHeadImgView.mentalImageDownRelease(urlStr: userModel.headPic, options: optionsInfo)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            UserFirstReactiveCompatible.received(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = TodayHandyJSON.deserialize(from: dic as? Dictionary) {
    ...
    private func juxtaposedNotify() {
        ...
        self.profileVC.distanceWith(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension KeyInputThen {
    ...
    @objc private func percentageBind() {
        //: req_followRequest()
        execute()
    }
    ...
    private func frame() {
        //: self.followBtn.isHidden = (userInfoModel.uid == StatisticalTableReactiveCompatible.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == StatisticalTableReactiveCompatible.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: showDataFormat.map{marginOfSafety(interest: $0)}, encoding: .utf8)! : (String(showCleanTitle.suffix(6)) + String(const_coverTitle))
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.barName(name: followImgStr)
    }
    ...
    private func setupSkinView() {
        ...
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = StatisticalTableReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = StatisticalTableReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.sinceMoment(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.sinceMoment(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension KeyInputThen: JXSegmentedViewDelegate {
...
extension KeyInputThen: JXPagingViewDelegate {
...
extension KeyInputThen {
    ...
    private func report() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(main_userContent)
        }
    }
    ...
    private func counterchange() {
        ...
        pagerView.frame = CGRect(x: 0, y: 0, width: appUseMessage, height: kStatusName)
        ...
        voiceRoomBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-actualHeight(h: 178) - constBarStr)
    ...
    private func priceNotifications() {
        // 个人信息更新成功刷新UI
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(refreshUserInfoNotification),
                                               selector: #selector(countLive),
    ...
    private func withMagnitude() {
        ...
        voiceRoomBtn.rx.tap
            ...
            .subscribe(onNext: { [weak self] in
                //: guard let self = self else {return}
                guard let self = self else { return }
                //: EditPushManager.share.func_pushToVoiceRoomVC(roomId: self.userInfoModel.partyRoomId.stringValue)
                EditPushManager.share.popMake(roomId: self.userInfoModel.partyRoomId.stringValue)
                //: })
            })
