...
fileprivate let noti_touchValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_home_shadow" :*/
fileprivate let noti_recordBoxMessage:String = "img_hvalue frame let pic info"
fileprivate let kMessagePlayId:[Character] = ["o","m","e","_","s","h","a","d","o","w"]

/*: "#9610FF" :*/
fileprivate let k_shareApplicationMessage:String = "#9610FFin tap if mask"

/*: "#8566FF" :*/
fileprivate let constLayerMsg:[Character] = ["#","8"]
fileprivate let constModelName:[Character] = ["5","6","6","F","F"]

/*: "Random Video" :*/
fileprivate let dataStrengthId:[Character] = ["R","a","n","d","o","m"," ","V","i"]
fileprivate let const_dataValue:String = "makeeo"

/*: "icon_match_call_free" :*/
fileprivate let const_equalStr:String = "icon_minput view manager pi"
fileprivate let noti_bottomFormat:String = "all_frecell to content like"
fileprivate let data_countValue:String = "listen"

/*: "Free" :*/
fileprivate let const_keyPath:[Character] = ["F","r","e","e"]

/*: "Scroll_upto_discove" :*/
fileprivate let main_pathData:String = "model and makeScrol"
fileprivate let app_nameData:String = "o_dispic app height"
fileprivate let noti_minId:String = "ccentere"

/*: "icon_VectorStroke" :*/
fileprivate let appMainStr:String = "icon_Vecount data do cell"
fileprivate let appMediumName:String = "quickor"

/*: "get json error" :*/
fileprivate let appTitleFormat:String = "make sendget "
fileprivate let showEaseData:String = "RROR"
...
@objc protocol ColorViewDelegate: NSObjectProtocol {
    ...
    @objc optional func outsideSecond(cardView: OrientationView, cardViewCell: TopRecognizerDelegate, forRowAtIndex: Int)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func compartmentWill(cardViewCell: TopRecognizerDelegate, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func messageEqual(cardViewCell: TopRecognizerDelegate, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func tarotCard(cardView: OrientationView, cardViewCell: TopRecognizerDelegate, forMovePoint: CGPoint)
    /// 随机匹配视频
    //: @objc optional func clubCardRandowBtnClick()
    @objc optional func moveMatch()
}
...
class OrientationView: UIView {
    ...
    var maxRemoveDistance = appUseMessage / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [TopRecognizerDelegate]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: DataMixObjectProtocol? /// 数据源
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_touchValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.barName(name: (String(noti_recordBoxMessage.prefix(5)) + String(kMessagePlayId)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.atArray(colors: [UIColor(hex: (String(k_shareApplicationMessage.prefix(7))))!.cgColor, UIColor(hex: (String(constLayerMsg) + String(constModelName)))!.cgColor], size: CGSize(width: appUseMessage - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(dataStrengthId) + const_dataValue.replacingOccurrences(of: "make", with: "d")).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(willClick), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.barName(name: (String(const_equalStr.prefix(6)) + "atch_c" + String(noti_bottomFormat.prefix(7)) + data_countValue.replacingOccurrences(of: "listen", with: "e"))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(const_keyPath)).localized, for: .normal)
...
extension OrientationView {
    ...
    private func simultaneously() {
        ...
        image.image = UIImage.barName(name: (String(main_pathData.suffix(5)) + "l_upt" + String(app_nameData.prefix(5)) + noti_minId.replacingOccurrences(of: "center", with: "ov")))
        ...
        image2.image = UIImage.barName(name: (String(appMainStr.prefix(7)) + appMediumName.replacingOccurrences(of: "quick", with: "ct") + "Stroke"))
    ...
    private func viewPlayer(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = RequestThen.default.littleness(type: .Club_guidePage)
            ...
            printLog(message: (String(appTitleFormat.suffix(4)) + "json e" + showEaseData.lowercased()))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func willClick() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.moveMatch?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func blind(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.clickImage(cardView: self) ?? 0
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? TopRecognizerDelegate(), at: 0)
    ...
    func everyVideoAnim(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: TopRecognizerDelegate = self.visibleCells[i] as! TopRecognizerDelegate
            ...
            if isVisbleLast {
                ...
                self.delegate?.messageEqual?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                logModel(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func logModel(cell: TopRecognizerDelegate, transform: CGAffineTransform) {
    ...
    func air(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func fun(identifier: String) -> TopRecognizerDelegate? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! ToItemCell.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: ToItemCell = cellclass.init(reuseIdentifier: identifier)
    ...
    func northSouthDirection(direction: OverdoSwipeDirection) {
        //: if self.visibleCells.count == 0 { return }
        if self.visibleCells.count == 0 { return }
        //: let topcell = self.visibleCells.last as? TalkingClubCardViewCell
        let topcell = self.visibleCells.last as? TopRecognizerDelegate
...
extension OrientationView: CustomCellDelegete {
    //: func cardViewCellDidRemoveFromSuperView(cell: TalkingClubCardViewCell) {
    func putOnData(cell: TopRecognizerDelegate) {
        //: moveCount += 1
        moveCount += 1
        ...
        self.delegate?.outsideSecond?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.clickImage(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.compartmentWill?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
...
extension OrientationView {
    ...
    func dismissByBtn() {
        ...
        let index = DigitiserSendable.randow
