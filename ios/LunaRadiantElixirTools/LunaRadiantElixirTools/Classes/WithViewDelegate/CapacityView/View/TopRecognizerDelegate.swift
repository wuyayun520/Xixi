...
fileprivate let app_blockData:[UInt8] = [0xf1,0xf6,0xf1,0xfc,0xb0,0xeb,0xf7,0xec,0xed,0xfa,0xc2,0xb1,0xa8,0xf0,0xe9,0xfb,0xa8,0xf6,0xf7,0xfc,0xa8,0xea,0xed,0xed,0xf6,0xa8,0xf1,0xf5,0xf8,0xf4,0xed,0xf5,0xed,0xf6,0xfc,0xed,0xec]

fileprivate func governReceive(cell num: UInt8) -> UInt8 {
...
enum OverdoSwipeDirection: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol CustomCellDelegete: NSObjectProtocol {
    ///
...
class TopRecognizerDelegate: UIView {
    ...
    var cell_delegate: CustomCellDelegete?

    //: var model: TalkingMatchResultModel?
    var model: MakeMixModelType?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_blockData.map{governReceive(cell: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(jabbing(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension TopRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    @objc func jabbing(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if CapacityLanguageManager.shared.direction == .rightToLeft {
                ...
                if movePoint.x > appUseMessage / 4 {
                    ...
                    self.cell_delegate?.suiteState(cell: self, forMovePoint: self.currentPoint)
    ...
    func unsaddle() {
        ...
        self.cell_delegate?.putOnData(cell: self)
    }

    /// 还原卡片位置
    //: func restoreCellLocation() {
    func refreshFillSolarSystem() {
    ...
    func outspoken(direction: OverdoSwipeDirection) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
