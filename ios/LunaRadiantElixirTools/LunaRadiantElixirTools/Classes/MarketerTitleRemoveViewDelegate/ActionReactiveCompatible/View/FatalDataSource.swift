...
fileprivate let show_modelMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
enum SystemHashableRepresentation: Int {
    //: case TimeGap = 0
...
@objc protocol MotionObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func getDownWithinScroll()

    //: @objc optional func endScroll()
    @objc optional func bean()
}

//: class TalkingDanmuMsgListTableView: UIView {
class FatalDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_modelMsg.reversed(), encoding: .utf8)!)
    }
...
extension FatalDataSource {
    ...
    func inhabit() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(makeEqual), userInfo: nil, repeats: true)
    }
...
extension FatalDataSource {
    ...
    func cellModel(msgModel: PresentationMeasurable?) {
    ...
    func addPop() {
        ...
        if AllHeight > kUseName {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < kUseName {
                ...
                self.msgTableView.height = kUseName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kUseName - AllHeight
        ...
        viewAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func viewAnimated(animated: Bool) {
...
extension FatalDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.delegate?.getDownWithinScroll?()
    }
    ...
    func alleywayCagyDetail(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(data_videoStr)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = SeeReactiveCompatible.calculate(tableView: tableView, msg: msgModel, indexPath: indexPath)
