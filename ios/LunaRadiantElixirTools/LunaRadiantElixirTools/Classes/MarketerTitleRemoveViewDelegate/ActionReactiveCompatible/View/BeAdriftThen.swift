...
fileprivate let k_dataTitle:[UInt8] = [0x39,0x3e,0x39,0x24,0x78,0x33,0x3f,0x34,0x35,0x22,0x6a,0x79,0x70,0x38,0x31,0x23,0x70,0x3e,0x3f,0x24,0x70,0x32,0x35,0x35,0x3e,0x70,0x39,0x3d,0x20,0x3c,0x35,0x3d,0x35,0x3e,0x24,0x35,0x34]

private func clickHead(bar num: UInt8) -> UInt8 {
    return num ^ 80
}

/*: "move" :*/
fileprivate let app_removeTitleFormat:String = "moon"
...
class BeAdriftThen: UIView {
    ...
    var danumuMsgArray: [DataScreenModel] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [CreateItemView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<DataScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_dataTitle.map{clickHead(bar: $0)}, encoding: .utf8)!)
    }
    ...
    func dataClick(_ danumuMsgModel: DataScreenModel?) {
    ...
    @objc func makeFor(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(makeFor(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension BeAdriftThen: UIGestureRecognizerDelegate {
    ...
    func half(for danumuMsgModel: DataScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = CreateItemView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (app_removeTitleFormat.replacingOccurrences(of: "on", with: "ve")))
    ...
    func superiorChannel(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                half(for: model, isFirst: isFirst)
