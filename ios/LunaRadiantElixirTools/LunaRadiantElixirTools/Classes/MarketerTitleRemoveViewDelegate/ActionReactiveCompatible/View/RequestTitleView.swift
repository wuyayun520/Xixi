...
fileprivate let userStyleMsg:[UInt8] = [0xb7,0xb0,0xb7,0xaa,0xf6,0xbd,0xb1,0xba,0xbb,0xac,0xe4,0xf7,0xfe,0xb6,0xbf,0xad,0xfe,0xb0,0xb1,0xaa,0xfe,0xbc,0xbb,0xbb,0xb0,0xfe,0xb7,0xb3,0xae,0xb2,0xbb,0xb3,0xbb,0xb0,0xaa,0xbb,0xba]

private func modePathInfo(layer num: UInt8) -> UInt8 {
    return num ^ 222
}

/*: "Select mute duration" :*/
fileprivate let data_addViewValue:[Character] = ["S","e","l","e","c","t"," ","m","u","t","e"," ","d","u","r","a","t","i","o"]
fileprivate let show_giftTitle:[Character] = ["n"]

/*: "Cirform" :*/
fileprivate let appCornerMessage:[Character] = ["C","i","r","f","o","r","m"]

/*: "Mute success" :*/
fileprivate let kThanMessage:String = "class size server buttonMute"

/*: "day" :*/
fileprivate let mainViewStr:[Character] = ["d","a","y"]

/*: "days" :*/
fileprivate let dataLabStr:String = "dsucceedys"

/*:  " :*/
fileprivate let kTrackTitle:String = " "
...
class RequestTitleView: UIView {
    ...
    var viewType: BreedViewType = .live
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userStyleMsg.map{modePathInfo(layer: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.font = UIFont.finishFont(fontSize: 18)
        //: lb.textColor = UIColor.appTitleColor()
        lb.textColor = UIColor.blindConstraint()
        //: lb.textAlignment = .center
        lb.textAlignment = .center
        //: lb.text = "Select mute duration".localized
        lb.text = (String(data_addViewValue) + String(show_giftTitle)).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(appCornerMessage)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(valueShared), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RequestTitleView {
    ...
    func resourceView() {
        ...
        self.contentView.frame = CGRect(x: (appUseMessage - contentV_Size.width) / 2, y: kStatusName, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (kStatusName - self.contentV_Size.height) / 2
        }
    }
    ...
    func outGreet() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = kStatusName
...
extension RequestTitleView {
    ...
    @objc private func refer(btn: UIButton) {
    ...
    @objc private func valueShared() {
        ...
        if self.viewType == .live {
            ...
            UpReactiveCompatible().pastFinish(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                BeanNameProgressHUD.doingBlock()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.taproom(showMsg: (String(kThanMessage.suffix(4)) + " success").localized)
                }
                //: self.dismiss()
                self.outGreet()
            }
            ...
            FirstEqualReqTool.each(targetUid: targetUid, roomID: DrawSaloonReactiveCompatible.signShared().partyModel.roomId, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                BeanNameProgressHUD.doingBlock()
                //: if succeed {
                if succeed {
...
extension RequestTitleView {
    ...
    private func makeConstraint() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.mastheadState(color: UIColor.readModeAchromaticColour(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (String(mainViewStr)).localized : (dataLabStr.replacingOccurrences(of: "succeed", with: "a")).localized
