...
/*: "id" :*/
fileprivate let notiTitleMsg:[Character] = ["i","d"]

/*: "num" :*/
fileprivate let noti_pushMsg:String = "ntag"
...
@objc public enum ReportSourceKeyRepresentable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct GiftHandyJSON: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// ReportSourceKeyRepresentable
    ...
    mutating func bestowal(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ExitRawAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (noti_pushMsg.replacingOccurrences(of: "tag", with: "um"))
    }
}
