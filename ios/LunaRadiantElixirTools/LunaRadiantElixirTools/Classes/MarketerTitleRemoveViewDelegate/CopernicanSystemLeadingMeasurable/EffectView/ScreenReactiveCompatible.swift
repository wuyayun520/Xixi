...
fileprivate let k_labelFinishForMessage:[UInt8] = [0x9f,0x98,0x9f,0x82,0xde,0x95,0x99,0x92,0x93,0x84,0xcc,0xdf,0xd6,0x9e,0x97,0x85,0xd6,0x98,0x99,0x82,0xd6,0x94,0x93,0x93,0x98,0xd6,0x9f,0x9b,0x86,0x9a,0x93,0x9b,0x93,0x98,0x82,0x93,0x92]

private func tableData(empty num: UInt8) -> UInt8 {
    return num ^ 246
}

/*: "iosSvgaEffect" :*/
fileprivate let noti_greetLoadAddStr:String = "mic iniosSvgaE"
fileprivate let kManagerTitle:String = "ffebar"

/*: "iosEffect" :*/
fileprivate let app_strengthKey:String = "iosEffmanager view succeed cell"
fileprivate let main_socialMsg:[Character] = ["e","c","t"]

/*: "entityId" :*/
fileprivate let show_frameMsg:String = "entityIdview cell info hidden image"
...
protocol ConsumerStepProtocol: NSObject {
    //: func animatViewWithStartLoadingResource(effectView: TalkingGiftAnimatView, obj: TalkingAnimatMsgModel)
    func eventWithObj(effectView: ScreenReactiveCompatible, obj: CopernicanSystemLeadingMeasurable)

    //: func animatViewWithDidEndLoadingResource(effectView: TalkingGiftAnimatView, obj: TalkingAnimatMsgModel, success: Bool)
    func modelInfo(effectView: ScreenReactiveCompatible, obj: CopernicanSystemLeadingMeasurable, success: Bool)

    //: func animatViewWithStartAnimating(effectView: TalkingGiftAnimatView, obj: TalkingAnimatMsgModel, success: Bool)
    func weltanschauungVideo(effectView: ScreenReactiveCompatible, obj: CopernicanSystemLeadingMeasurable, success: Bool)

    //: func animatViewWithDidFinishAnimating(effectView: TalkingGiftAnimatView, obj: TalkingAnimatMsgModel)
    func messageObj(effectView: ScreenReactiveCompatible, obj: CopernicanSystemLeadingMeasurable)
}

//: typealias AnimationFinishCallback = (_ obj: TalkingAnimatMsgModel) -> Void
typealias AnimationFinishCallback = (_ obj: CopernicanSystemLeadingMeasurable) -> Void
...
class ScreenReactiveCompatible: UIView {
    ...
    var currentEffectMsgModel: CopernicanSystemLeadingMeasurable?
    ...
    var animatedView: MixItemView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: ConsumerStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_labelFinishForMessage.map{tableData(empty: $0)}, encoding: .utf8)!)
    }
...
extension ScreenReactiveCompatible {
    ...
    func formatting(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(noti_greetLoadAddStr.suffix(8)) + kManagerTitle.replacingOccurrences(of: "bar", with: "ct"))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(app_strengthKey.prefix(6)) + String(main_socialMsg))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(show_frameMsg.prefix(8)))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = textTheory(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func textTheory(msgModel: Any?) -> Bool {
        ...
        if msgModel is CopernicanSystemLeadingMeasurable {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? CopernicanSystemLeadingMeasurable
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is GiftHandyJSON {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! GiftHandyJSON
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.bestowal(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = CopernicanSystemLeadingMeasurable()
            ...
            effectMsgModel = CopernicanSystemLeadingMeasurable()
            ...
            if giftMsgModel.showType == ReportSourceKeyRepresentable.myStery.rawValue || giftMsgModel.showType == ReportSourceKeyRepresentable.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: CopernicanSystemLeadingMeasurable = self.effectMsgModelArray[i] as! CopernicanSystemLeadingMeasurable
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? CopernicanSystemLeadingMeasurable())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? CopernicanSystemLeadingMeasurable(), at: index)
                }
            ...
            premiereFromTheoreticalAccount(effectMsgModel: effectMsgModel!)
        }
...
extension ScreenReactiveCompatible {
    ...
    func viewResume() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = recordModel() else { return }
                //: firstShow(effectMsgModel: removeModel)
                premiereFromTheoreticalAccount(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.inkingPad()
        }
    ...
    func recordModel() -> CopernicanSystemLeadingMeasurable? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return CopernicanSystemLeadingMeasurable()
        }
        ...
        if effectMsgModel is CopernicanSystemLeadingMeasurable {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! CopernicanSystemLeadingMeasurable
        ...
        return effectMsgModel as? CopernicanSystemLeadingMeasurable
    }
}
...
extension ScreenReactiveCompatible {
    ...
    func premiereFromTheoreticalAccount(effectMsgModel: CopernicanSystemLeadingMeasurable) {
        ...
        self.delegate?.eventWithObj(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        UserAnimatTool.shared.forename(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.atHandle(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? DateTransformable())
            //: })
        })
    }
    ...
    func atHandle(effectMsgModel: CopernicanSystemLeadingMeasurable, success: Bool, effectModel: DateTransformable) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.modelInfo(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    circleModel(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func aboveFrom(effectMsgModel: CopernicanSystemLeadingMeasurable, effectModel: DateTransformable) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.frank()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.successRecord()
        ...
        className = UserAnimatTool.shared.infoByName(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! MixItemView.Type
...
extension ScreenReactiveCompatible: MixViewDelegate {
    ...
    func resultContentMake(effectItemView: MixItemView, success: Bool) {
        ...
        if !success {
            ...
            successFinishAddApp(success: success)
        }
        ...
        self.delegate?.weltanschauungVideo(effectView: self, obj: effectItemView.effectMsgModel ?? CopernicanSystemLeadingMeasurable(), success: success)
    }
    ...
    func decrease(effectItemView: MixItemView) {
        ...
        if effectItemView == self.animatedView {
            ...
            if !self.needStopAll {
                ...
                successFinishAddApp(success: true)
            }
        }
        ...
        self.delegate?.messageObj(effectView: self, obj: effectMsgModel!)
    }
}
...
extension ScreenReactiveCompatible {
    ...
    func serviceData() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.frank() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.successRecord() // clean animation data
    ...
    func successFinishAddApp(success: Bool) {
        //: cleanData()
        serviceData()
        ...
        if self.effectMsgModelArray.count > 0, self.needStopAll == false {
            ...
            let effectMsgModel = recordModel()
