...
fileprivate let noti_targetCellText:[UInt8] = [0x72,0x75,0x72,0x6f,0x33,0x78,0x74,0x7f,0x7e,0x69,0x21,0x32,0x3b,0x73,0x7a,0x68,0x3b,0x75,0x74,0x6f,0x3b,0x79,0x7e,0x7e,0x75,0x3b,0x72,0x76,0x6b,0x77,0x7e,0x76,0x7e,0x75,0x6f,0x7e,0x7f]

private func numberrySave(tag num: UInt8) -> UInt8 {
    return num ^ 27
}

/*: "%@.mp4" :*/
fileprivate let constEqualPathTitle:String = "%@.mp4false true icon error add"

/*: "http" :*/
fileprivate let noti_leadingUrl:[Character] = ["h","t","t","p"]
...
class ResultAnimationView: MixItemView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_targetCellText.map{numberrySave(tag: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(toGroup))
    ...
    override func findOut() {
        ...
        let path = UserAnimatTool.shared.cardTranslate(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? CopernicanSystemLeadingMeasurable())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(constEqualPathTitle.prefix(6))), path)
...
extension ResultAnimationView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((String(noti_leadingUrl))) {
