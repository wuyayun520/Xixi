...
fileprivate let showSizeUrl:[UInt8] = [0x22,0x25,0x22,0x3f,0x63,0x28,0x24,0x2f,0x2e,0x39,0x71,0x62,0x6b,0x23,0x2a,0x38,0x6b,0x25,0x24,0x3f,0x6b,0x29,0x2e,0x2e,0x25,0x6b,0x22,0x26,0x3b,0x27,0x2e,0x26,0x2e,0x25,0x3f,0x2e,0x2f]

private func tempColor(cell num: UInt8) -> UInt8 {
    return num ^ 75
}
...
class SlateTableView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(tapMakeAction))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showSizeUrl.map{tempColor(cell: $0)}, encoding: .utf8)!)
    }
...
extension SlateTableView {
    ...
    func effectModel(msgModel: EffectHandyJSON?) {
    ...
    func requestEnable() {
        ...
        if AllHeight > show_detailId {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < show_detailId {
                ...
                self.msgTableView.height = show_detailId
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = show_detailId - AllHeight
        ...
        undersurfaceAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func undersurfaceAnimated(animated: Bool) {
...
extension SlateTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func timeButton(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.beautyScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(user_explainUrl)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = AtViewCell.errorItem(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension SlateTableView {
    ...
    func duringView() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: show_failVoiceData, height: show_detailId)
    }
}
