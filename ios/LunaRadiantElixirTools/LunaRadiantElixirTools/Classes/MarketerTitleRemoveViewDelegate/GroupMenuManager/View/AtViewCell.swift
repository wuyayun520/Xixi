...
fileprivate let app_mainMarginId:[UInt8] = [0x53,0x58,0x53,0x5e,0x12,0x4d,0x59,0x4e,0x4f,0x5c,0x24,0x13,0xa,0x52,0x4b,0x5d,0xa,0x58,0x59,0x5e,0xa,0x4c,0x4f,0x4f,0x58,0xa,0x53,0x57,0x5a,0x56,0x4f,0x57,0x4f,0x58,0x5e,0x4f,0x4e]

fileprivate func ballsUp(info num: UInt8) -> UInt8 {
...
/*: _ :*/
fileprivate let show_strokeLinePath:String = "_"

/*: "UITableViewCell" :*/
fileprivate let app_tabMessage:String = "UITaapp first"
fileprivate let userPartyName:[Character] = ["b","l","e","V","i","e","w","C","e","l","l"]
...
class AtViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.msgLabel.snp.makeConstraints { make in
            ...
            make.top.equalTo(user_explainUrl + 2)
...
//            make.trailing.lessThanOrEqualTo(-32)
        ...
        self.bgLb.snp.makeConstraints { make in
            ...
            make.top.equalTo(user_explainUrl)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_mainMarginId.map{ballsUp(info: $0)}, encoding: .utf8)!)
    }
...
extension AtViewCell {
    ...
    class func errorItem(tableView: UITableView, msg: EffectHandyJSON, indexPath _: IndexPath) -> AtViewCell {
        ...
        if msg.messageType == 1 {
            ...
            let identifier = "\(MotionVideoBassCell.className())_\(msg.messageType)"
            //: tableView.register(TalkingVideoDanmuMsgTextCell.self, forCellReuseIdentifier: identifier)
            tableView.register(MotionVideoBassCell.self, forCellReuseIdentifier: identifier)
            //: cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingVideoDanmuMsgTextCell
            cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? MotionVideoBassCell
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgTextCell(style: .default, reuseIdentifier: identifier)
                cell = MotionVideoBassCell(style: .default, reuseIdentifier: identifier)
            }
            //: }else {
        } else {
            //: let identifier = "UITableViewCell"
            let identifier = (String(app_tabMessage.prefix(4)) + String(userPartyName))
