...
fileprivate let showMakeValue:[UInt8] = [0xf1,0xf6,0xf1,0xec,0xb0,0xfb,0xf7,0xfc,0xfd,0xea,0xa2,0xb1,0xb8,0xf0,0xf9,0xeb,0xb8,0xf6,0xf7,0xec,0xb8,0xfa,0xfd,0xfd,0xf6,0xb8,0xf1,0xf5,0xe8,0xf4,0xfd,0xf5,0xfd,0xf6,0xec,0xfd,0xfc]

private func showArray(request num: UInt8) -> UInt8 {
    return num ^ 152
}

/*: "#FFD13A" :*/
fileprivate let const_loadMsg:String = "name wrap height#FFD1"
fileprivate let mainMakeName:String = "3Amiddle color mode to equal"

/*: "gift_gb_umnad_1" :*/
fileprivate let data_ageMsg:[Character] = ["g","i","f","t","_","g","b","_","u","m","n","a","d","_","1"]

/*: "icon_circle_pic_default" :*/
fileprivate let user_modelTableLabelId:String = "list color title in videoicon_c"
fileprivate let userImageFormat:String = "equal equale_pic_"
fileprivate let kInputText:String = "dat"
fileprivate let main_fromText:String = "momentult"

/*: "Open lucky blind-box" :*/
fileprivate let data_valueFormat:[Character] = ["O","p","e","n"," ","l","u","c","k","y"," ","b","l","i","n"]
fileprivate let dataColorFormat:[Character] = ["d","-","b","o","x"]

/*: "Sent " :*/
fileprivate let main_equalTitle:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let const_readRouteViewId:String = "Pingname let return"
fileprivate let appStopMessage:[Character] = ["S","e","m","i","b","o","l","d"]

/*: "%d" :*/
fileprivate let appCombineFormat:String = "adjust to%d"

/*: "Bombard" :*/
fileprivate let main_noteUrl:[Character] = ["B","o","m","b","a","r","d"]
...
class MonthThen: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = GiftHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showMakeValue.map{showArray(request: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .statusDataMove(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(const_loadMsg.suffix(5)) + String(mainMakeName.prefix(2))))
    ...
    private lazy var pluralTip: DropThen = {
    ...
    private lazy var countLb: DropThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = DropThen()
...
extension MonthThen {
    ...
    func bottomBlock(model: GiftHandyJSON) {
        ...
        self.bgImage.image = UIImage.barName(name: (String(data_ageMsg)))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.barName(name: (String(user_modelTableLabelId.suffix(6)) + "ircl" + String(userImageFormat.suffix(6)) + kInputText.replacingOccurrences(of: "at", with: "e") + main_fromText.replacingOccurrences(of: "moment", with: "fa")))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ReportSourceKeyRepresentable.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(data_valueFormat) + String(dataColorFormat)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.mentalImageDownRelease(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.mentalImageDownRelease(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(main_equalTitle)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(const_readRouteViewId.prefix(4)) + "FangSC-" + String(appStopMessage)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = listSum(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(const_readRouteViewId.prefix(4)) + "FangSC-" + String(appStopMessage)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = listSum(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.seededPlayer(texts: strs, colors: colors, fonts: fonts)
        ...
        plus(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func plus(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(const_readRouteViewId.prefix(4)) + "FangSC-" + String(appStopMessage)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = listSum(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(const_readRouteViewId.prefix(4)) + "FangSC-" + String(appStopMessage)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = listSum(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.buttonThemeTip(150, 77, 20, 1), UIColor.white, UIColor.buttonThemeTip(150, 77, 20, 1)]
        ...
        attText = attText.seededPlayer(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.thanBy(attributedText: attText)
        ...
        countLb.alongSumerval(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func comboThroughPossibilityModel(giftMsgModel: GiftHandyJSON) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(roundWindow), object: nil)
        ...
        self.perform(#selector(roundWindow), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func sleepingPill(model _: GiftHandyJSON, animate _: Bool) {
    ...
    func analogDigitalConverterStatus(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
    ...
    @objc func roundWindow() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(roundWindow), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = GiftHandyJSON()
...
extension MonthThen {
    ...
    func listSum(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension MonthThen {
    ...
    private func equalRe() {
        ...
        font = listSum(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.seededPlayer(texts: [(String(main_noteUrl)).localized], colors: [UIColor.buttonThemeTip(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.alongSumerval(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.thanBy(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.alongSumerval(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
