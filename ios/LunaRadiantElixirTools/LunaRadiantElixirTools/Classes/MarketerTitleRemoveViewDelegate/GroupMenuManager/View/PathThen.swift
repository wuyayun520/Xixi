...
fileprivate let k_sizePhotoValue:[UInt8] = [0xcb,0xcc,0xcb,0xd6,0x8a,0xc1,0xcd,0xc6,0xc7,0xd0,0x98,0x8b,0x82,0xca,0xc3,0xd1,0x82,0xcc,0xcd,0xd6,0x82,0xc0,0xc7,0xc7,0xcc,0x82,0xcb,0xcf,0xd2,0xce,0xc7,0xcf,0xc7,0xcc,0xd6,0xc7,0xc6]

private func itemM(list num: UInt8) -> UInt8 {
    return num ^ 162
}
...
class PathThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_sizePhotoValue.map{itemM(list: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        MakeSocketDelegate.shared.containerView?.isHidden = false
    }

    /// 移除视图
    //: func dismissWhenCallEnd() {
    func numbereractionDestroy() {
        //: removeAllGestures()
        everyLastUp()
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= appUseMessage * 0.5) ? -self.width : appUseMessage + self.width
            ...
            MakeSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension PathThen {
    ...
    func rangeEnd() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.liveSender(sender:)))
    ...
    func gestures() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.viewLive(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.rangeEnd()
    ...
    @objc func viewLive(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(appUseMessage - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(kStatusName - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = appUseMessage - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = appUseMessage
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= appUseMessage * 0.5 {
            ...
            MakeSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func liveSender(sender _: UIPanGestureRecognizer) {
