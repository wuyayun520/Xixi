...
/*: "call_bgm" :*/
fileprivate let const_cellInfoMessage:String = "cavoice"

/*: "call_response_bgm" :*/
fileprivate let show_statusUrl:String = "call_relab value height"
fileprivate let const_scaleTitle:[Character] = ["s","p","o","n","s","e","_","b","g","m"]
...
enum RenameEqualBreedConstantTarget: Int {
    //: case Call
    case Call /// 音视频拨打音乐
    //: case Response
...
class StatusReactiveCompatible: NSObject {
    ...
    var type = RenameEqualBreedConstantTarget.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func maine(type: RenameEqualBreedConstantTarget) -> StatusReactiveCompatible {
        //: let player = TalkingCallBGMPlayer()
        let player = StatusReactiveCompatible()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.backBy()
...
extension StatusReactiveCompatible {
    ...
    private func handleWith(notify: Notification) {
    ...
    private func dataConverterRead(notify: Notification) {
    ...
    private func displaceEvent() -> Bool {
    ...
    private func backBy() {
        ...
        var fileName = (const_cellInfoMessage.replacingOccurrences(of: "voice", with: "ll") + "_bgm")
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(show_statusUrl.prefix(7)) + String(const_scaleTitle))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = RequestThen.default.clickVertical(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension StatusReactiveCompatible: TXVodPlayListener {
