...
fileprivate let constSizeValue:String = "make path readTSystemMe"
fileprivate let data_toAwakeKey:String = "messagemessageag"
fileprivate let mainSucceedValue:String = "center size share self varl_ReuseId"

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiLogValue:[UInt8] = [0x24,0x29,0x24,0x2f,0xe3,0x1e,0x2a,0x1f,0x20,0x2d,0xf5,0xe4,0xdb,0x23,0x1c,0x2e,0xdb,0x29,0x2a,0x2f,0xdb,0x1d,0x20,0x20,0x29,0xdb,0x24,0x28,0x2b,0x27,0x20,0x28,0x20,0x29,0x2f,0x20,0x1f]

fileprivate func videoFrame(my num: UInt8) -> UInt8 {
...
/*: "icon_talk_notic" :*/
fileprivate let constActualId:String = "icon_taframe external"
fileprivate let user_areaMsg:[Character] = ["l","k","_","n","o","t","i","c"]
...
@objc public protocol MakeGroup: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: BroadcastThen)
    func toBackground(cell: BroadcastThen)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: BroadcastThen)
    func equalOf(cell: BroadcastThen)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: BroadcastThen)
    func stage(cell: BroadcastThen)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: BroadcastThen)
    func nest(cell: BroadcastThen)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: BroadcastThen)
    func compartmentContextCell(cell: BroadcastThen)
}

//: @objcMembers public class BroadcastThen: TCommonTableViewCell {
@objcMembers public class BroadcastThen: ToReactiveCompatible {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\ReloadViewCellData.h
     */
    //: public var messageData: ReloadViewCellData?
    public var messageData: ReloadViewCellData?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(constSizeValue.suffix(9)) + data_toAwakeKey.replacingOccurrences(of: "message", with: "s") + "eCel" + String(mainSucceedValue.suffix(9)))

    /**
     *  协议委托
     *  负责实现 MakeGroup 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: MakeGroup?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiLogValue.map{videoFrame(my: $0)}, encoding: .utf8)!)
    }
    ...
    override public func changeDestroyWith(with data: NameUpReactiveCompatible) {
        //: super.fill(with: data)
        super.changeDestroyWith(with: data)
        //: let newData: ReloadViewCellData = data as! ReloadViewCellData
        let newData: ReloadViewCellData = data as! ReloadViewCellData
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.voiceNote()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(upRecognizer(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.finishFont(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(source(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(access(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.barName(name: (String(constActualId.prefix(7)) + String(user_areaMsg)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(gesturePlayerRecognizer(recognizer:)))
