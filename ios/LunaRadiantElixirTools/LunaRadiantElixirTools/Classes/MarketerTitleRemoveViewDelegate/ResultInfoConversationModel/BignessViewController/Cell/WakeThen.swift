...
/*: "#ffffff" :*/
fileprivate let data_showName:[Character] = ["#","f","f","f","f","f"]
fileprivate let const_imageValue:[Character] = ["f"]

/*: "earned" :*/
fileprivate let showMakeStr:String = "eaconversationned"

/*: "Coins in" :*/
fileprivate let dataFrameUrl:[Character] = ["C"]
fileprivate let user_modelPositionText:String = "lab heightoins in"

/*: "Error:  :*/
fileprivate let userMessageUrl:String = "to first clickError: "

/*: "#C946D2" :*/
fileprivate let main_modelUrl:String = "#C946D2link you frame cancel"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let constSubItemValue:String = "progress path add live viewbtn_"
fileprivate let appSucceedId:String = "hidden key style app hypothesisfloatS_us"
fileprivate let k_toTextPath:[Character] = ["e","r","B","K","_","n","o","r"]
...
class WakeThen: UIView {
    ...
    private let TextColor = (String(data_showName) + String(const_imageValue))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: ColorModelType?) {
        //: self.init()
        self.init()
    ...
    private func endAcross() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            temporaryWorker(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (showMakeStr.replacingOccurrences(of: "conversation", with: "r"))) {
                ...
                if let range = attrStr.string.range(of: (String(dataFrameUrl) + String(user_modelPositionText.suffix(7)))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.colorTitle(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(userMessageUrl.suffix(7))) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func temporaryWorker(withUrl url: String) {
        ...
        self.backgroundImageView.mentalImageDownRelease(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.busy(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: ColorModelType? {
        //: didSet {
        didSet {
            //: configModel()
            endAcross()
    ...
    var totalTime: CGFloat {
        ...
        return (appUseMessage + widthValue) * SpendTime / appUseMessage
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / appUseMessage
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / appUseMessage
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(main_modelUrl.prefix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.barName(name: (String(constSubItemValue.suffix(4)) + "chat_" + String(appSucceedId.suffix(9)) + String(k_toTextPath)))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(needAction(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension WakeThen {
    ...
    @objc func needAction(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = StatisticalTableReactiveCompatible.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = StatisticalTableReactiveCompatible.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: EditPushManager.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        EditPushManager.share.methodConfig(urlStr: httpsUrl, webConfig: config)
    }
}
