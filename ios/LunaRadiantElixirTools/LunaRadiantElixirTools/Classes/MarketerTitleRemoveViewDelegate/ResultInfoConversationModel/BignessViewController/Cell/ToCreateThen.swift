...
fileprivate let k_profileName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_lock" :*/
fileprivate let dataNumberMsg:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let app_listStr:String = "session view labt_lock"

/*: "icon_chat_burned" :*/
fileprivate let notiNameStr:String = "data path add data ificon_"
fileprivate let appShareTitle:String = "self lab_burned"

/*: "icon_chat_burned_shan" :*/
fileprivate let k_rowValue:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u","r","n","e","d","_","s","h","a"]
fileprivate let main_pathData:String = "N"

/*: "Already burned" :*/
fileprivate let user_layerName:String = "equal objectAlread"
...
class ToCreateThen: ReloadMessageCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_profileName.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.barName(name: (String(dataNumberMsg) + String(app_listStr.suffix(6))))
    ...
    lazy var burnedImagV: TalkingItemButton = {
        ...
        imgV.setBackgroundImage(UIImage.barName(name: (String(notiNameStr.suffix(5)) + "chat" + String(appShareTitle.suffix(7)))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.barName(name: (String(k_rowValue) + main_pathData.lowercased())), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(user_layerName.suffix(6)) + "y burned").localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.statusDataMove(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ToCreateThen {
    ...
    override func changeDestroyWith(with data: NameUpReactiveCompatible) {
        ...
        let intimatePhoto: StorageResponseModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            showFirst(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            showFirst(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: ItemCellData, urlStr: String?, isMosaic: Bool) {
    func showFirst(data: ItemCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.mild(image, typeVideoLevel: 90)
                        self.photoView.image = UIImage.mild(image, typeVideoLevel: 90)
                        //: } else {
                    } else {
            ...
            afterConversation(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.sinceMoment(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func afterConversation(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.arrangeInsideSupplement(urlStr: urlStr) { [weak self] image in
