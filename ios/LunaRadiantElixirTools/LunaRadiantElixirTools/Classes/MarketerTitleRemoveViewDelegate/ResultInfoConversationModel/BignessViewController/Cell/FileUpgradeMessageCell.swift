...
fileprivate let dataSucceedDateValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_miss" :*/
fileprivate let main_acceptMatchKey:String = "extension view return noticon_"
fileprivate let constAAppName:[Character] = ["s","s"]

/*: "icon_chat_yes" :*/
fileprivate let dataColorValue:String = "strength viewicon_c"
...
class FileUpgradeMessageCell: ReloadMessageCell {
//    var transBlock: (() -> Void)?

    //: var textData: AttributeCellData?
    var textData: AttributeCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataSucceedDateValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var quoteView: InfoTopView = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = InfoTopView()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(correlationTableCount), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != MixInReactiveCompatible.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.add() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.barName(name: (String(main_acceptMatchKey.suffix(5)) + "chat_mi" + String(constAAppName)))
                ...
                self.transStatusImagV.image = UIImage.barName(name: (String(dataColorValue.suffix(6)) + "hat_yes"))
            }
    ...
    func pastGes() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(nameAdd(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MixInReactiveCompatible.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MixInReactiveCompatible.messageTo())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            pub()
...
//            }
    ...
    func pub() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(compartmentTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func nameAdd(tap _: UITapGestureRecognizer?) {
    ...
    @objc func compartmentTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func correlationTableCount() {
        //: self.delegate?.onReplyMessage(cell: self)
        self.delegate?.compartmentContextCell(cell: self)
    }
}
