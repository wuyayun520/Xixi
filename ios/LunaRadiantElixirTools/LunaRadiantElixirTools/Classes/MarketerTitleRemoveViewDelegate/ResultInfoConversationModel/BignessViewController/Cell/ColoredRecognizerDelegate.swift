...
fileprivate let constTitleKey:[UInt8] = [0xf0,0xf7,0xf0,0xed,0xb1,0xfa,0xf6,0xfd,0xfc,0xeb,0xa3,0xb0,0xb9,0xf1,0xf8,0xea,0xb9,0xf7,0xf6,0xed,0xb9,0xfb,0xfc,0xfc,0xf7,0xb9,0xf0,0xf4,0xe9,0xf5,0xfc,0xf4,0xfc,0xf7,0xed,0xfc,0xfd]

private func endMan(point num: UInt8) -> UInt8 {
    return num ^ 153
}

/*: "move" :*/
fileprivate let notiVideoPath:String = "moframe"
...
// __CLOSE_PRINT__
//
//  ColoredRecognizerDelegate.swift
//  AbroadTalking
//
...
class ColoredRecognizerDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constTitleKey.map{endMan(point: $0)}, encoding: .utf8)!)
    }
    ...
    func popReceive(_ danumuMsgModel: ColorModelType?) {
    ...
    @objc func alienation(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(alienation(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension ColoredRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    func breakIn(for danumuMsgModel: ColorModelType?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = WakeThen(model: danumuMsgModel)
        ...
        var frame = CGRect(x: appUseMessage, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if CapacityLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = appUseMessage
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (notiVideoPath.replacingOccurrences(of: "frame", with: "ve")))
    ...
    func auditoryImage(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                breakIn(for: model, isFirst: isFirst)
...
extension ColoredRecognizerDelegate {
    // 添加视图
