...
fileprivate let constHideName:[UInt8] = [0x78,0x7f,0x78,0x65,0x39,0x72,0x7e,0x75,0x74,0x63,0x2b,0x38,0x31,0x79,0x70,0x62,0x31,0x7f,0x7e,0x65,0x31,0x73,0x74,0x74,0x7f,0x31,0x78,0x7c,0x61,0x7d,0x74,0x7c,0x74,0x7f,0x65,0x74,0x75]

/*: "icon_chat_translate_nor" :*/
fileprivate let show_backgroundLabName:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r"]
fileprivate let const_modelMessage:String = "ansname"
fileprivate let noti_sendUrl:[Character] = ["a","t","e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let dataQuickName:String = "icon_cas color"
fileprivate let noti_makeStr:String = "ranperson"
fileprivate let main_blockFormat:String = "path"

/*: "icon_chat_miss" :*/
fileprivate let k_modelMsg:String = "to course manualicon_c"

/*: "icon_chat_yes" :*/
fileprivate let showFromModeMessage:String = "icon_chaif succeed view push"
fileprivate let appDataKey:[Character] = ["t","_","y","e","s"]
...
class ToViewCell: ReloadMessageCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: PerspectiveThen?
    var textData: PerspectiveThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constHideName.map{$0^17}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.statusDataMove(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.barName(name: (String(show_backgroundLabName) + const_modelMessage.replacingOccurrences(of: "name", with: "l") + String(noti_sendUrl))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.barName(name: (String(dataQuickName.prefix(6)) + "hat_t" + noti_makeStr.replacingOccurrences(of: "person", with: "s") + "late_pr" + main_blockFormat.replacingOccurrences(of: "path", with: "e"))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(appTransView), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ToViewCell {
    ...
    override func changeDestroyWith(with data: NameUpReactiveCompatible) {
        ...
        //: textData = data as? PerspectiveThen
        textData = data as? PerspectiveThen
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.sinceMoment(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = MixInReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = MixInReactiveCompatible.statusByPlay() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.barName(name: (String(k_modelMsg.suffix(6)) + "hat_miss"))
            ...
            self.transStatusImagV.image = UIImage.barName(name: (String(showFromModeMessage.prefix(8)) + String(appDataKey)))
        }
    ...
    func fromGes() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(system(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func system(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
