...
//: video
@property (nonatomic, copy) NSString *kLiveFormat;

//: Sent 
@property (nonatomic, copy) NSString *appCostText;

//: img
@property (nonatomic, copy) NSString *mainLogIndexData;

//: text
@property (nonatomic, copy) NSString *appFilterLiveUrl;

//: gift
@property (nonatomic, copy) NSString *noti_allAddMsg;

//: audio
@property (nonatomic, copy) NSString *notiCropStr;
...
- (Byte *)MediaDataToCache:(Byte *)data {
    int lite = data[0];
    Byte block = data[1];
    int cropPublish = data[2];
    for (int i = cropPublish; i < cropPublish + lite; i++) {
    ...
    data[cropPublish + lite] = 0;
    return data + cropPublish;
}
...
//: audio
- (NSString *)notiCropStr {
    if (!_notiCropStr) {
        Byte value[] = {5, 25, 12, 189, 69, 211, 52, 204, 183, 182, 232, 122, 122, 142, 125, 130, 136, 43};
        _notiCropStr = [self StringFromMediaData:value];
    }
    return _notiCropStr;
}

//: text
- (NSString *)appFilterLiveUrl {
    if (!_appFilterLiveUrl) {
        Byte value[] = {4, 84, 9, 44, 6, 84, 81, 111, 229, 200, 185, 204, 200, 242};
        _appFilterLiveUrl = [self StringFromMediaData:value];
    }
    return _appFilterLiveUrl;
}

//: img
- (NSString *)mainLogIndexData {
    if (!_mainLogIndexData) {
        Byte value[] = {3, 75, 9, 110, 188, 48, 115, 156, 243, 180, 184, 178, 29};
        _mainLogIndexData = [self StringFromMediaData:value];
    }
    return _mainLogIndexData;
}

//: Sent 
- (NSString *)appCostText {
    if (!_appCostText) {
        Byte value[] = {5, 21, 12, 140, 246, 47, 53, 68, 222, 173, 209, 211, 104, 122, 131, 137, 53, 243};
        _appCostText = [self StringFromMediaData:value];
    }
    return _appCostText;
}

//: video
- (NSString *)kLiveFormat {
    if (!_kLiveFormat) {
        Byte value[] = {5, 89, 12, 55, 117, 196, 45, 213, 221, 192, 224, 50, 207, 194, 189, 190, 200, 233};
        _kLiveFormat = [self StringFromMediaData:value];
    }
    return _kLiveFormat;
}

//: gift
- (NSString *)noti_allAddMsg {
    if (!_noti_allAddMsg) {
        Byte value[] = {4, 95, 7, 112, 237, 73, 174, 198, 200, 197, 211, 217};
        _noti_allAddMsg = [self StringFromMediaData:value];
    }
    return _noti_allAddMsg;
}
...
@implementation StorageResponseModel
...
+ (void)withId:(StorageResponseModel *)photoModel toId:(NSString *)msgID {
    ...
    NSMutableDictionary *dict = [self orientation].mutableCopy;
    //: NSString *jsonString = [photoModel toJSONString];
    NSString *jsonString = [photoModel toJSONString];
    //: dict[msgID] = jsonString;
    dict[msgID] = jsonString;
    //: NSString *path = [OpenReactiveCompatible getIntimatePhotoPath];
    NSString *path = [OpenReactiveCompatible indexOn];
...
+ (void)onEye:(ByModel *)videoModel deliverForUnwantedData:(NSString *)msgID {
    ...
    NSMutableDictionary *dict = [self doing].mutableCopy;
    //: NSString *jsonString = [videoModel toJSONString];
    NSString *jsonString = [videoModel toJSONString];
    //: dict[msgID] = jsonString;
    dict[msgID] = jsonString;
    //: NSString *path = [OpenReactiveCompatible getIntimateVideoPath];
    NSString *path = [OpenReactiveCompatible listStatus];
    //: [NSKeyedArchiver archiveRootObject:dict toFile:path];
    [NSKeyedArchiver archiveRootObject:dict toFile:path];
}
...
@implementation ComputerSimulationModel
...
- (CGSize)theDataSize:(NSInteger)maxWidth{
    ...
    NSDictionary* attributes = @{NSFontAttributeName: [UIFont statusDataMoveWithFontSize:12]};


    //: if ([self.renderType isEqualToString:@"video"] || [self.renderType isEqualToString:@"img"]) {
    if ([self.renderType isEqualToString:[MediaData sharedInstance].kLiveFormat] || [self.renderType isEqualToString:[MediaData sharedInstance].mainLogIndexData]) {
    ...
    if ([self.renderType isEqualToString:[MediaData sharedInstance].appFilterLiveUrl]) {
    ...
    if ([self.renderType isEqualToString:[MediaData sharedInstance].notiCropStr]) {
        //: NSString *str = [NSString stringWithFormat:@"%@: ",self.sendName];
        NSString *str = [NSString stringWithFormat:@"%@: ",self.sendName];
        //: CGSize nameSize = [str boundingRectWithSize:CGSizeMake(1.7976931348623157e+308, 14) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
        CGSize nameSize = [str boundingRectWithSize:CGSizeMake(1.7976931348623157e+308, 14) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
    ...
    if ([self.renderType isEqualToString:[MediaData sharedInstance].noti_allAddMsg]) {
        ...
        NSString *str = [NSString stringWithFormat:@"%@: %@%@ x%ld",self.sendName,[MediaData sharedInstance].appCostText.drownLevel,self.renderData.catScan, (long)self.renderData.giftNum];
        //: CGSize contentSize = [str boundingRectWithSize:CGSizeMake(1.7976931348623157e+308, 14) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
        CGSize contentSize = [str boundingRectWithSize:CGSizeMake(1.7976931348623157e+308, 14) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
        //: return CGSizeMake(((maxWidth) < (5 + contentSize.width) ? (maxWidth) : (5 + contentSize.width)) , 14);
        return CGSizeMake(((maxWidth) < (5 + contentSize.width) ? (maxWidth) : (5 + contentSize.width)) , 14);
...
@implementation StorageSizeModel
...
- (NSString *)catScan{
    //: NSString *locName;
    NSString *locName;
    //: NSString *sysLangCode = [MixInReactiveCompatible getSystemLangCode];
    NSString *sysLangCode = [MixInReactiveCompatible withEvent];
...
+ (void)shrink:(NSString *)msgID {
    ...
    @synchronized (self) {
        ...
        NSMutableArray *arr = [self textModel];
        //: [arr addObject:msgID];
        [arr addObject:msgID];
        //: NSString *path = [OpenReactiveCompatible getAskForGiftPath];
        NSString *path = [OpenReactiveCompatible pausePath];
