...
fileprivate let const_bottomLabFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let data_layerKey:String = "kit"
fileprivate let const_viewMapId:String = "未知消\u{606f}]"

/*: "extra" :*/
fileprivate let noti_sourceMessage:[Character] = ["e","x","t","r","a"]

/*: "</at> " :*/
fileprivate let k_toData:String = "equal let index cell</at> "

/*: "#0084FF" :*/
fileprivate let appPathAtMessage:String = "#learn84"
fileprivate let kReSelectedMsg:String = "model"

/*: "#FFEE4B" :*/
fileprivate let kGiftTitle:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let mainModeKey:String = "style index var model<at>"
fileprivate let mainButtonTitle:String = "wrap let class countrys]+?</at"
fileprivate let user_arrayTitle:[Character] = [">"]

/*: "jumpKey" :*/
fileprivate let showOfEqualTitle:[Character] = ["j","u","m","p","K","e","y"]

/*: "user" :*/
fileprivate let k_makeMessageKey:[Character] = ["u","s","e","r"]

/*: "uid" :*/
fileprivate let dataColorMsg:[UInt8] = [0x80,0x9c,0x91]

private func bridePrice(label num: UInt8) -> UInt8 {
    return num ^ 245
}

/*: "#16D073" :*/
fileprivate let main_appIconRowKey:[Character] = ["#","1","6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let k_originMsg:String = "open if self message intimate#F95644"
...
//: @objcMembers public class PerspectiveThen: TalkingChatMsgBaseCellData {
@objcMembers public class PerspectiveThen: AggregationThen {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_bottomLabFormat.reversed(), encoding: .utf8)!)
    }
    ...
    override public var msgModel: OverModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (data_layerKey.replacingOccurrences(of: "kit", with: "[") + const_viewMapId)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = bastardyModel(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func bastardyModel(msgInfoModel: MainModel) -> NSMutableAttributedString {
        ...
        guard dic[(String(noti_sourceMessage))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.blindConstraint() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (appPathAtMessage.replacingOccurrences(of: "learn", with: "00") + kReSelectedMsg.replacingOccurrences(of: "model", with: "FF")))! : UIColor(hex: (String(kGiftTitle)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(mainModeKey.suffix(4)) + "@[\\S\\" + String(mainButtonTitle.suffix(8)) + String(user_arrayTitle)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(showOfEqualTitle))].stringValue == (String(k_makeMessageKey)) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: dataColorMsg.map{bridePrice(label: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: dataColorMsg.map{bridePrice(label: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func subName() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(main_appIconRowKey)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(k_originMsg.suffix(7))))!
        }
