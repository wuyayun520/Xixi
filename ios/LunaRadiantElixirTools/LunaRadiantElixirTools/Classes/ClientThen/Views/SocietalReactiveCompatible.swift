...
fileprivate let user_textValue:[UInt8] = [0x68,0x6f,0x68,0x75,0x29,0x62,0x6e,0x65,0x64,0x73,0x3b,0x28,0x21,0x69,0x60,0x72,0x21,0x6f,0x6e,0x75,0x21,0x63,0x64,0x64,0x6f,0x21,0x68,0x6c,0x71,0x6d,0x64,0x6c,0x64,0x6f,0x75,0x64,0x65]

private func tableData(normal num: UInt8) -> UInt8 {
    return num ^ 1
}

/*: "icon_rank_coin" :*/
fileprivate let kCloseKey:[UInt8] = [0xbd,0xb7,0xc3,0xc2,0xb3,0xc6,0xb5,0xc2,0xbf,0xb3,0xb7,0xc3,0xbd,0xc2]

fileprivate func indexShowManager(indicator num: UInt8) -> UInt8 {
...
/*: "icon_lounge" :*/
fileprivate let noti_factorPath:String = "icon_loumodel return right table"
fileprivate let mainRequestPath:String = "ngdata"

/*: "icon_rank_coin2" :*/
fileprivate let mainItemMsg:String = "icon_content var status price return"
fileprivate let notiAddContent:[Character] = ["2"]
...
class SocietalReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_textValue.map{tableData(normal: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.shouldHiddenAppear()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .colorTitle(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = TalkingItemButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.barName(name: String(bytes: kCloseKey.map{indexShowManager(indicator: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .colorTitle(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.atArray(colors: UIColor.govern(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.barName(name: (String(noti_factorPath.prefix(8)) + mainRequestPath.replacingOccurrences(of: "data", with: "e")))
    }
}
...
extension SocietalReactiveCompatible {
    ...
    func infoAdd(model: CustomHandyJSON, index: Int) {
        ...
        headPic.sinceMoment(urlStr: model.headPic)
    ...
    func statusModifyType(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: kCloseKey.map{indexShowManager(indicator: $0)}, encoding: .utf8)! : (String(mainItemMsg.prefix(5)) + "rank_coin" + String(notiAddContent))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.barName(name: imageStr), for: .normal)
    }
}
