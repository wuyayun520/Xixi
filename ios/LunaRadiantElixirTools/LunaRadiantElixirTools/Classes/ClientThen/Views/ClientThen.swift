...
fileprivate let data_equalEffectMessage:[UInt8] = [0x1c,0x1b,0x1c,0x1,0x5d,0x16,0x1a,0x11,0x10,0x7,0x4f,0x5c,0x55,0x1d,0x14,0x6,0x55,0x1b,0x1a,0x1,0x55,0x17,0x10,0x10,0x1b,0x55,0x1c,0x18,0x5,0x19,0x10,0x18,0x10,0x1b,0x1,0x10,0x11]

private func modelResult(to num: UInt8) -> UInt8 {
    return num ^ 117
}

/*: "Not yet~" :*/
fileprivate let main_insertLeadName:String = "Not yet~path record info index data"

/*: "#8C7AFF" :*/
fileprivate let main_arrayTitle:String = "start range back view window#8C7AFF"
...
class ClientThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.constraintCellMutual()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_equalEffectMessage.map{modelResult(to: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [CustomHandyJSON] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(FileView.self, forCellReuseIdentifier: FileView.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(SocietalReactiveCompatible.self, forCellReuseIdentifier: SocietalReactiveCompatible.className())
    }
    ...
    lazy var tableHeaderView = ImaginationImageReactiveCompatible(frame: CGRect(x: 0, y: 0, width: appUseMessage, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = UnveilThen().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(main_insertLeadName.prefix(8))).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(main_arrayTitle.suffix(7))))
...
extension ClientThen: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: FileView.className(), for: indexPath) as! FileView
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: SocietalReactiveCompatible.className(), for: indexPath) as! SocietalReactiveCompatible
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.infoAdd(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.statusModifyType(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: FileView.className()) as! FileView
...
extension ClientThen: JXSegmentedListContainerViewListDelegate {
...
extension ClientThen {
    //: private func setupSubviews() {
    private func constraintCellMutual() {
