...
/*: "Search" :*/
fileprivate let app_addPresentationId:String = "Searchlet return finish content in"

/*: "#F5F5F5" :*/
fileprivate let kSmallSharedName:String = "frame action of name normal#F5F5F5"

/*: "Search by UID" :*/
fileprivate let dataNameStr:[Character] = ["S","e","a","r","c","h"," ","b","y"," ","U","I"]
fileprivate let main_labelMatchChangeData:[Character] = ["D"]

/*: "icon_search_sea" :*/
fileprivate let constDeadlinePath:[Character] = ["i","c","o","n","_","s","e","a","r"]
fileprivate let main_voiceImageStr:String = "ch_seaif return extension name cell"

/*: "Can’t find~" :*/
fileprivate let main_liveLabelStr:String = "color info false voice leftCan’"
fileprivate let data_bubbleName:[Character] = ["t"," ","f","i","n","d","~"]

/*: "icon_kong_kong_default" :*/
fileprivate let kCellKey:String = "icon_koresult player area"
fileprivate let dataButtonStr:[Character] = ["n","g","_","k","o","n","g","_","d"]
fileprivate let app_actionEmptyName:[Character] = ["e","f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let main_valueName:String = "var model model blockPlea"
fileprivate let appLabValue:String = "layer caseter "
fileprivate let app_videoClickData:String = "the querself table index"
fileprivate let k_makeName:[Character] = ["y"," ","U","I","D"]
...
class PopNameViewController: DropBaseViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(app_addPresentationId.prefix(6))).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 15)
        btn.titleLabel?.font = UIFont.statusDataMove(fontSize: 15)
        ...
        btn.setBackgroundImage(UIImage.atArray(colors: UIColor.govern(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(deviceAfter), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.placeholder = (String(dataNameStr) + String(main_labelMatchChangeData)).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.barName(name: (String(constDeadlinePath) + String(main_voiceImageStr.prefix(6))))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(EnableThen.self, forCellReuseIdentifier: EnableThen.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(main_liveLabelStr.suffix(4)) + String(data_bubbleName)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(kCellKey.prefix(7)) + String(dataButtonStr) + String(app_actionEmptyName))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + noti_noId, width: appUseMessage, height: kStatusName - (14 + noti_noId + user_viewSumervalKey)), style: style)
    ...
    private lazy var reqManager: MakeReactiveCompatible = //: return MakeReactiveCompatible()
        .init()
    //: }()
}
...
extension PopNameViewController {
    // 根据uid查询用户
    ...
    private func nameDown() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.alongShow(showMsg: (String(main_valueName.suffix(4)) + "se en" + String(appLabValue.suffix(4)) + String(app_videoClickData.prefix(8)) + String(k_makeName)).localized)
            //: return
            return
        }
        ...
        reqManager.productGift(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension PopNameViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: EnableThen.className(), for: indexPath) as! EnableThen
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.roundLabel(model: model)
...
extension PopNameViewController: UITextFieldDelegate {
...
extension PopNameViewController: JXPagingViewListViewDelegate {
