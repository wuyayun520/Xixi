...
class DanceProduct {
  ...
  const DanceProduct(this.amountStr, this.productId, this.price);
}

class DanceProducts {
  static const List<DanceProduct> all = [
    DanceProduct('32', 'Xixi', 0.99),
    DanceProduct('96', 'Xixi2', 2.99),
    DanceProduct('189', 'Xixi5', 5.99),
    DanceProduct('299', 'Xixi9', 9.99),
    DanceProduct('599', 'Xixi19', 19.99),
    DanceProduct('1599', 'Xixi49', 49.99),
    DanceProduct('3199', 'Xixi99', 99.99),
    DanceProduct('5099', 'Xixi159', 159.99),
    DanceProduct('7959', 'Xixi239', 239.99),
  ];
}
...
class _InAppPurchasesPageState extends State<InAppPurchasesPage> with TickerProviderStateMixin {
  ...
  List<DanceProduct> get _danceProducts => DanceProducts.all;
  ...
  void initState() {
    ...
    _danceAnimation = Tween<double>(begin: -0.1, end: 0.1).animate(
  ...
  Future<void> _loadDanceCoins() async {
    ...
    debugPrint('Loading dance coins: $coins');
    setState(() {
      _danceCoins = coins;
      if (!_isInitialized) {
        _isInitialized = true;
        debugPrint('Initialized from _loadDanceCoins');
      }
    });
  }

  Future<void> _saveDanceCoins(int amount) async {
    final prefs = await SharedPreferences.getInstance();
    debugPrint('Saving dance coins: current=$_danceCoins, adding=$amount, new=${_danceCoins + amount}');
  ...
  Future<void> _initInAppPurchase() async {
    ...
    final Set<String> productIds = _danceProducts.map((e) => e.productId).toSet();
  ...
  Future<void> _handleSuccessfulPurchase(PurchaseDetails purchaseDetails) async {
    ...
    debugPrint('Available product IDs: ${_danceProducts.map((p) => p.productId).toList()}');
    
    setState(() {
      _purchasePending = false;
    });
    
    final product = _danceProducts.firstWhere(
      (e) => e.productId == purchaseDetails.productID, 
      orElse: () {
        debugPrint('Product not found in configuration: ${purchaseDetails.productID}');
        return DanceProduct('', '', 0);
      }
    );
    
    if (product.amount > 0) {
      debugPrint('Processing purchase: ${product.amount} coins for product ${purchaseDetails.productID}');
      await _saveDanceCoins(product.amount);
      _showSnackBar("Purchase successful! +${product.amount} Dance Coins");
    } else {
                ...
                SliverPadding(
                  ...
                  sliver: SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        final item = _danceProducts[index];
                      ...
                      childCount: _danceProducts.length,
                    ),
                  ),
                ),
              ],
